...
public class TerrAttachHelper {
    ...
    private static com.pullenti.ner.ReferentToken _tryAttachPureTerr(java.util.ArrayList<TerrItemToken> li, com.pullenti.ner.core.AnalyzerData ad) {
        ...
        if (com.pullenti.ner.core.BracketHelper.canBeStartOfSequence(tt, true, false)) 
            tt = tt.getNext();
        if (li.size() > 1) {
            java.util.ArrayList<TerrItemToken> tmp = new java.util.ArrayList(li);
            tmp.remove(0);
            ...
            if (rt0 != null) {
                if ((((com.pullenti.ner.geo.GeoReferent)com.pullenti.n2j.Utils.cast(rt0.referent, com.pullenti.ner.geo.GeoReferent.class))).isState()) 
                    return null;
    ...
    public static com.pullenti.ner.ReferentToken tryAttachTerritory(java.util.ArrayList<TerrItemToken> li, com.pullenti.ner.core.AnalyzerData ad, boolean attachAlways, java.util.ArrayList<CityItemToken> cits, java.util.ArrayList<com.pullenti.ner.geo.GeoReferent> exists) {
        ...
        java.util.ArrayList<TerrItemToken> adjList = new java.util.ArrayList< >();
        TerrItemToken noun = null;
        TerrItemToken addNoun = null;
        com.pullenti.ner.ReferentToken rt = _tryAttachMoscowAO(li, ad);
        if (rt != null) 
            return rt;
        if (li.get(0).terminItem != null && com.pullenti.n2j.Utils.stringsEq(li.get(0).terminItem.getCanonicText(), "ТЕРРИТОРИЯ")) {
        ...
        for(k = 0; k < li.size(); k++) {
            if (li.get(k).ontoItem != null) {
                ...
                if (noun != null) {
                    if (k == 1) {
                        if (com.pullenti.n2j.Utils.stringsEq(noun.terminItem.getCanonicText(), "РАЙОН") || com.pullenti.n2j.Utils.stringsEq(noun.terminItem.getCanonicText(), "ОБЛАСТЬ") || com.pullenti.n2j.Utils.stringsEq(noun.terminItem.getCanonicText(), "СОЮЗ")) {
                            if (li.get(k).ontoItem.referent instanceof com.pullenti.ner.geo.GeoReferent) {
                                if ((((com.pullenti.ner.geo.GeoReferent)com.pullenti.n2j.Utils.cast(li.get(k).ontoItem.referent, com.pullenti.ner.geo.GeoReferent.class))).isState()) 
                                    break;
                            }
                            boolean ok = false;
                ...
                if (li.get(k).terminItem.isAdjective || li.get(k).isGeoInDictionary) 
                    ...
                    if (exObj != null) {
                        com.pullenti.ner.geo.GeoReferent _geo = (com.pullenti.ner.geo.GeoReferent)com.pullenti.n2j.Utils.cast(exObj.ontoItem.referent, com.pullenti.ner.geo.GeoReferent.class);
                        if (_geo == null) 
                            break;
                        if (exObj.isAdjective && ((com.pullenti.n2j.Utils.stringsEq(li.get(k).terminItem.getCanonicText(), "СОЮЗ") || com.pullenti.n2j.Utils.stringsEq(li.get(k).terminItem.getCanonicText(), "ФЕДЕРАЦИЯ")))) {
                            String str = exObj.ontoItem.toString();
                            if (!(str.indexOf(li.get(k).terminItem.getCanonicText()) >= 0)) 
                                return null;
                        }
                        if (com.pullenti.n2j.Utils.stringsEq(li.get(k).terminItem.getCanonicText(), "РАЙОН") || com.pullenti.n2j.Utils.stringsEq(li.get(k).terminItem.getCanonicText(), "ОКРУГ") || com.pullenti.n2j.Utils.stringsEq(li.get(k).terminItem.getCanonicText(), "КРАЙ")) {
                    ...
                    noun = li.get(k);
                    if (k == 0) {
                        TerrItemToken tt = TerrItemToken.tryParse(li.get(k).getBeginToken().getPrevious(), null, true, false);
        ...
        if (exObj != null) {
            ...
            if (noun == null) 
                return null;
            for(int j = 1; j < k; j++) {
                if (li.get(j).isNewlineBefore()) 
                    return null;
            }
            _morph = noun.getMorph();
            if (newName.isAdjective) {
                if (com.pullenti.n2j.Utils.stringsEq(noun.terminItem.acronym, "АО")) {
                ...
                if (!li.get(0).isNewlineBefore() && tt0 != null) 
                    geoBefore = (com.pullenti.ner.geo.GeoReferent)com.pullenti.n2j.Utils.cast(tt0.getReferent(), com.pullenti.ner.geo.GeoReferent.class);
                if (li.indexOf(noun) < li.indexOf(newName)) {
                    ...
                    if (rtp != null) 
                        return null;
                    name = com.pullenti.ner.core.ProperNameHelper.getName(newName.getBeginToken(), newName.getEndToken(), com.pullenti.morph.MorphClass.ADJECTIVE, com.pullenti.morph.MorphCase.UNDEFINED, noun.terminItem.getGender(), false, false);
                }
                else {
                    boolean ok = false;
                    if (((k + 1) < li.size()) && li.get(k).terminItem == null && li.get(k + 1).terminItem != null) 
                        ...
                        ok = true;
                    else if (MiscLocationHelper.checkGeoObjectAfter(li.get(k - 1).getEndToken())) 
                        ok = true;
                    ...
                    fullName = com.pullenti.ner.core.ProperNameHelper.getName(li.get(0).getBeginToken(), noun.getBeginToken().getPrevious(), com.pullenti.morph.MorphClass.ADJECTIVE, com.pullenti.morph.MorphCase.UNDEFINED, noun.terminItem.getGender(), false, false) + " " + noun.terminItem.getCanonicText();
                }
            }
            else {
                if (!attachAlways || ((noun.terminItem != null && com.pullenti.n2j.Utils.stringsEq(noun.terminItem.getCanonicText(), "ФЕДЕРАЦИЯ")))) {
                    boolean isLatin = noun.chars.isLatinLetter() && newName.chars.isLatinLetter();
                    if (li.indexOf(noun) > li.indexOf(newName)) {
                        if (!isLatin) 
                            return null;
                    }
                    if (!newName.isDistrictName) {
                        ...
                        if (!isLatin) {
                            ...
                            if (noun.isDoubt && adjList.size() == 0) {
                                if (((com.pullenti.n2j.Utils.stringsEq(noun.terminItem.acronym, "МО") || com.pullenti.n2j.Utils.stringsEq(noun.terminItem.acronym, "ЛО"))) && k == (li.size() - 1) && li.get(k).terminItem != null) {
                                    addNoun = li.get(k);
                                    k++;
                                }
                                else 
                ...
                name = com.pullenti.ner.core.ProperNameHelper.getName(newName.getBeginToken(), newName.getEndToken(), com.pullenti.morph.MorphClass.UNDEFINED, com.pullenti.morph.MorphCase.UNDEFINED, com.pullenti.morph.MorphGender.UNDEFINED, false, false);
                if (adjList.size() > 0) {
                    ...
                    if (npt != null && npt.getEndToken() == noun.getEndToken()) 
                        altName = npt.getNormalCaseText(new com.pullenti.morph.MorphClass(null), false, com.pullenti.morph.MorphGender.UNDEFINED, false) + " " + name;
                }
            }
        }
        else {
            if ((li.size() == 1 && noun != null && noun.getEndToken().getNext() != null) && (noun.getEndToken().getNext().getReferent() instanceof com.pullenti.ner.geo.GeoReferent)) {
                com.pullenti.ner.geo.GeoReferent g = (com.pullenti.ner.geo.GeoReferent)com.pullenti.n2j.Utils.cast(noun.getEndToken().getNext().getReferent(), com.pullenti.ner.geo.GeoReferent.class);
                if (noun.terminItem != null) {
                    if (g.findSlot(com.pullenti.ner.geo.GeoReferent.ATTR_TYPE, noun.terminItem.getCanonicText().toLowerCase(), true) != null) 
                        return com.pullenti.ner.ReferentToken._new709(g, noun.getBeginToken(), noun.getEndToken().getNext(), noun.getBeginToken().getMorph());
                }
            }
            if ((li.size() == 1 && noun == li.get(0) && li.get(0).terminItem != null) && TerrItemToken.tryParse(li.get(0).getEndToken().getNext(), null, true, false) == null && TerrItemToken.tryParse(li.get(0).getBeginToken().getPrevious(), null, true, false) == null) {
                ...
                for(com.pullenti.ner.Token tt = li.get(0).getBeginToken().getPrevious(); tt != null; tt = tt.getPrevious()) {
                    ...
                    if (cou > 500) 
                        break;
                    com.pullenti.ner.geo.GeoReferent g = (com.pullenti.ner.geo.GeoReferent)com.pullenti.n2j.Utils.cast(tt.getReferent(), com.pullenti.ner.geo.GeoReferent.class);
                    if (g == null) 
                        continue;
                    boolean ok = true;
                    cou = 0;
                    for(tt = li.get(0).getEndToken().getNext(); tt != null; tt = tt.getNext()) {
                        ...
                        if (cou > 500) 
                            break;
                        TerrItemToken tee = TerrItemToken.tryParse(tt, null, true, false);
                        if (tee == null) 
                            continue;
                        ok = false;
                        break;
                    ...
                    if (ok) {
                        for(int ii = 0; g != null && (ii < 3); g = g.getHigher(),ii++) {
                            if (g.findSlot(com.pullenti.ner.geo.GeoReferent.ATTR_TYPE, str, true) != null) 
                                return com.pullenti.ner.ReferentToken._new709(g, li.get(0).getBeginToken(), li.get(0).getEndToken(), noun.getBeginToken().getMorph());
                        }
                    }
                    break;
                }
        ...
        if (exObj != null && (exObj.tag instanceof com.pullenti.ner.geo.GeoReferent)) 
            ter = (com.pullenti.ner.geo.GeoReferent)com.pullenti.n2j.Utils.cast(exObj.tag, com.pullenti.ner.geo.GeoReferent.class);
        else {
            ...
            if (noun != null) {
                if (com.pullenti.n2j.Utils.stringsEq(noun.terminItem.getCanonicText(), "АО")) 
                    ter.addTyp((li.get(0).kit.baseLanguage.isUa() ? "АВТОНОМНИЙ ОКРУГ" : "АВТОНОМНЫЙ ОКРУГ"));
                else if (com.pullenti.n2j.Utils.stringsEq(noun.terminItem.getCanonicText(), "МУНИЦИПАЛЬНОЕ СОБРАНИЕ") || com.pullenti.n2j.Utils.stringsEq(noun.terminItem.getCanonicText(), "МУНІЦИПАЛЬНЕ ЗБОРИ")) 
                    ter.addTyp((li.get(0).kit.baseLanguage.isUa() ? "МУНІЦИПАЛЬНЕ УТВОРЕННЯ" : "МУНИЦИПАЛЬНОЕ ОБРАЗОВАНИЕ"));
                else if (com.pullenti.n2j.Utils.stringsEq(noun.terminItem.acronym, "МО") && addNoun != null) 
        ...
        if ((res.getBeginToken().getPrevious() instanceof com.pullenti.ner.TextToken) && (res.getWhitespacesBeforeCount() < 2)) {
            com.pullenti.ner.TextToken tt = (com.pullenti.ner.TextToken)com.pullenti.n2j.Utils.cast(res.getBeginToken().getPrevious(), com.pullenti.ner.TextToken.class);
            if (com.pullenti.n2j.Utils.stringsEq(tt.term, "АР")) {
    ...
    public static com.pullenti.ner.ReferentToken tryAttachStateUSATerritory(com.pullenti.ner.Token t) {
        ...
        if (tok == null) 
            return null;
        com.pullenti.ner.geo.GeoReferent g = (com.pullenti.ner.geo.GeoReferent)com.pullenti.n2j.Utils.cast(tok.termin.tag, com.pullenti.ner.geo.GeoReferent.class);
        if (g == null) 
            return null;
