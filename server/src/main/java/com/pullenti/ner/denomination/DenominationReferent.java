...
public class DenominationReferent extends com.pullenti.ner.Referent {
    ...
    public String toString(boolean shortVariant, com.pullenti.morph.MorphLang lang, int lev) {
        return (String)com.pullenti.n2j.Utils.notnull(getValue(), "?");
    }
    ...
    public void addValue(com.pullenti.ner.Token begin, com.pullenti.ner.Token end) {
        ...
        addSlot(ATTR_VALUE, tmp.toString(), false, 0);
        m_Names = null;
    }

    @Override
    public boolean canBeEquals(com.pullenti.ner.Referent obj, com.pullenti.ner.Referent.EqualType typ) {
        DenominationReferent dr = (DenominationReferent)com.pullenti.n2j.Utils.cast(obj, DenominationReferent.class);
        if (dr == null) 
            return false;
    ...
    private java.util.ArrayList<String> getNameVars() {
        if (m_Names != null) 
            return m_Names;
        m_Names = new java.util.ArrayList< >();
        String nam = getValue();
        if (nam == null) 
            return m_Names;
        m_Names.add(nam);
        java.util.ArrayList<java.util.ArrayList<String>> items = new java.util.ArrayList< >();
        int i;
        int ty0 = 0;
        int i0 = 0;
    ...
    private static void addVars(String str, java.util.ArrayList<String> vars) {
        ...
        for(int k = 0; k < 2; k++) {
            ...
            for(i = 0; i < str.length(); i++) {
                String v;
                com.pullenti.n2j.Outargwrapper<String> inoutarg1060 = new com.pullenti.n2j.Outargwrapper< >();
                Boolean inoutres1061 = com.pullenti.n2j.Utils.tryGetValue(m_VarChars, str.charAt(i), inoutarg1060);
                v = inoutarg1060.value;
                if (!inoutres1061) 
                    break;
    ...
    static {
        m_VarChars = new java.util.HashMap< >();
        m_VarChars.put('A', "АА");
        m_VarChars.put('B', "БВ");
        m_VarChars.put('C', "ЦС");
        m_VarChars.put('D', "ДД");
