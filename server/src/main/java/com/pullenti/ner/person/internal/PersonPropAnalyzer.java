...
public class PersonPropAnalyzer extends com.pullenti.ner.Analyzer {
    ...
    public com.pullenti.ner.ReferentToken processReferent(com.pullenti.ner.Token begin, com.pullenti.ner.Token end) {
        PersonAttrToken pat = PersonAttrToken.tryAttach(begin, null, PersonAttrToken.PersonAttrAttachAttrs.NO);
        if (pat != null && pat.getPropRef() != null) 
            return com.pullenti.ner.ReferentToken._new2382(pat.getPropRef(), pat.getBeginToken(), pat.getEndToken(), pat.getMorph(), pat);
        return null;
    }
}
