...
public class PersonIdToken extends com.pullenti.ner.MetaToken {
    ...
    public static com.pullenti.ner.ReferentToken tryAttach(com.pullenti.ner.Token t) {
        ...
        if (noun == null) 
            return null;
        java.util.ArrayList<PersonIdToken> li = new java.util.ArrayList< >();
    ...
    private static PersonIdToken tryParse(com.pullenti.ner.Token t, PersonIdToken prev) {
        if (t.isValue("СВИДЕТЕЛЬСТВО", null)) {
            ...
            for(com.pullenti.ner.Token tt = t.getNext(); tt != null; tt = tt.getNext()) {
                ...
                if (tt.isValue("РЕГИСТРАЦИЯ", null) || tt.isValue("РЕЕСТР", null) || tt.isValue("ЗАРЕГИСТРИРОВАТЬ", null)) {
                    ...
                    tt1 = tt;
                else if (tt.getReferent() != null && com.pullenti.n2j.Utils.stringsEq(tt.getReferent().getTypeName(), "DATERANGE")) 
                    tt1 = tt;
                else 
                    break;
            }
            if (reg && ip) 
                return _new2294(t, tt1, Typs.KEYWORD, "СВИДЕТЕЛЬСТВО О ГОСУДАРСТВЕННОЙ РЕГИСТРАЦИИ ФИЗИЧЕСКОГО ЛИЦА В КАЧЕСТВЕ ИНДИВИДУАЛЬНОГО ПРЕДПРИНИМАТЕЛЯ");
        }
        ...
        if (tok != null) {
            ...
            PersonIdToken res = _new2294(tok.getBeginToken(), tok.getEndToken(), ty, tok.termin.getCanonicText());
            if (prev == null) {
                if (ty != Typs.KEYWORD) 
                    return null;
                    ...
                    if (ait != null) {
                        if (ait.referent != null) {
                            for(com.pullenti.ner.Slot s : ait.referent.getSlots()) {
                                if (com.pullenti.n2j.Utils.stringsEq(s.getTypeName(), com.pullenti.ner.person.PersonPropertyReferent.ATTR_REF) && (s.getValue() instanceof com.pullenti.ner.geo.GeoReferent)) 
                                    res.referent = (com.pullenti.ner.Referent)com.pullenti.n2j.Utils.cast(s.getValue(), com.pullenti.ner.Referent.class);
                            }
                        }
                ...
                if ((res.referent instanceof com.pullenti.ner.geo.GeoReferent) && !(((com.pullenti.ner.geo.GeoReferent)com.pullenti.n2j.Utils.cast(res.referent, com.pullenti.ner.geo.GeoReferent.class))).isState()) 
                    res.referent = null;
                return res;
            }
            if (ty == Typs.NUMBER) {
        ...
        if (t instanceof com.pullenti.ner.NumberToken) {
            StringBuilder tmp = new StringBuilder();
            PersonIdToken res = _new2296(t0, t, Typs.NUMBER);
        ...
        if (t instanceof com.pullenti.ner.ReferentToken) {
            ...
            if (r != null) {
                if (com.pullenti.n2j.Utils.stringsEq(r.getTypeName(), "DATE")) 
                    return _new2297(t, t, Typs.DATE, r);
                if (com.pullenti.n2j.Utils.stringsEq(r.getTypeName(), "ORGANIZATION")) 
                    return _new2297(t, t, Typs.ORG, r);
                if (com.pullenti.n2j.Utils.stringsEq(r.getTypeName(), "ADDRESS")) 
                    return _new2297(t, t, Typs.ADDRESS, r);
            }
        }
        if ((prev != null && prev.typ == Typs.KEYWORD && (t instanceof com.pullenti.ner.TextToken)) && !t.chars.isAllLower() && t.chars.isLetter()) {
            ...
            if (rr != null && rr.typ == Typs.NUMBER) 
                return _new2294(t, t, Typs.SERIA, (((com.pullenti.ner.TextToken)com.pullenti.n2j.Utils.cast(t, com.pullenti.ner.TextToken.class))).term);
        }
        if ((t != null && t.isValue("ОТ", "ВІД") && (t.getNext() instanceof com.pullenti.ner.ReferentToken)) && com.pullenti.n2j.Utils.stringsEq(t.getNext().getReferent().getTypeName(), "DATE")) 
            return _new2297(t, t.getNext(), Typs.DATE, t.getNext().getReferent());
        return null;
    }
    ...
    public static void initialize() {
        ...
        t = com.pullenti.ner.core.Termin._new118("ПАСПОРТ", Typs.KEYWORD);
        t.addVariant("ПАССПОРТ", false);
        t.addVariant("ПАСПОРТНЫЕ ДАННЫЕ", false);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new118("ЗАГРАНИЧНЫЙ ПАСПОРТ", Typs.KEYWORD);
        t.addVariant("ЗАГРАНПАСПОРТ", false);
        t.addAbridge("ЗАГРАН. ПАСПОРТ");
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new118("УДОСТОВЕРЕНИЕ ЛИЧНОСТИ", Typs.KEYWORD);
        t.addVariant("УДОСТОВЕРЕНИЕ ЛИЧНОСТИ ОФИЦЕРА", false);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new118("СВИДЕТЕЛЬСТВО О ГОСУДАРСТВЕННОЙ РЕГИСТРАЦИИ ФИЗИЧЕСКОГО ЛИЦА В КАЧЕСТВЕ ИНДИВИДУАЛЬНОГО ПРЕДПРИНИМАТЕЛЯ", Typs.KEYWORD);
        ...
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new118("ВОДИТЕЛЬСКОЕ УДОСТОВЕРЕНИЕ", Typs.KEYWORD);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new118("ЛИЦЕНЗИЯ", Typs.KEYWORD);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new118("СЕРИЯ", Typs.SERIA);
        t.addAbridge("СЕР.");
        t.addVariant("СЕРИ", false);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new118("НОМЕР", Typs.NUMBER);
        t.addAbridge("НОМ.");
        t.addAbridge("Н-Р");
        t.addVariant("№", false);
        t.addVariant("N", false);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new118("ВЫДАТЬ", Typs.VIDAN);
        t.addVariant("ВЫДАВАТЬ", false);
        t.addVariant("ДАТА ВЫДАЧИ", false);
        t.addVariant("ДАТА РЕГИСТРАЦИИ", false);
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new118("КОД ПОДРАЗДЕЛЕНИЯ", Typs.CODE);
        t.addAbridge("К/П");
        t.addAbridge("К.П.");
        m_Ontology.add(t);
        t = com.pullenti.ner.core.Termin._new118("РЕГИСТРАЦИЯ", Typs.ADDRESS);
    ...
    public static PersonIdToken _new2294(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, Typs _arg3, String _arg4) {
        PersonIdToken res = new PersonIdToken(_arg1, _arg2);
        res.typ = _arg3;
        res.value = _arg4;
        return res;
    }
    public static PersonIdToken _new2296(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, Typs _arg3) {
        PersonIdToken res = new PersonIdToken(_arg1, _arg2);
        res.typ = _arg3;
        return res;
    }
    public static PersonIdToken _new2297(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, Typs _arg3, com.pullenti.ner.Referent _arg4) {
        PersonIdToken res = new PersonIdToken(_arg1, _arg2);
        res.typ = _arg3;
        res.referent = _arg4;
        return res;
