...
public class MailAnalyzer extends com.pullenti.ner.Analyzer {
    ...
    public com.pullenti.ner.Referent createReferent(String type) {
        if (com.pullenti.n2j.Utils.stringsEq(type, MailReferent.OBJ_TYPENAME)) 
    ...
    public void process(com.pullenti.ner.core.AnalysisKit kit) {
        java.util.ArrayList<com.pullenti.ner.mail.internal.MailLine> lines = new java.util.ArrayList< >();
        ...
        if (lines.size() == 0) 
            return;
        int i;
        java.util.ArrayList<java.util.ArrayList<com.pullenti.ner.mail.internal.MailLine>> blocks = new java.util.ArrayList< >();
        ...
        for(i = 0; i < lines.size(); i++) {
                ...
                if (!isNew) {
                    for(com.pullenti.ner.Token tt = ml.getBeginToken(); tt != null && tt.endChar <= ml.endChar; tt = tt.getNext()) {
                        if (tt.getReferent() != null) {
                            if (com.pullenti.n2j.Utils.stringsEq(tt.getReferent().getTypeName(), "DATE") || com.pullenti.n2j.Utils.stringsEq(tt.getReferent().getTypeName(), "URI")) 
                                isNew = true;
                        }
                    }
                }
                if (isNew) {
                    blk = new java.util.ArrayList< >();
                    blocks.add(blk);
                    for(; i < lines.size(); i++) {
            ...
            if (blk == null) 
                blocks.add((blk = new java.util.ArrayList< >()));
            blk.add(lines.get(i));
        }
        if (blocks.size() == 0) 
            return;
        ...
        for(int j = 0; j < blocks.size(); j++) {
            ...
            if (lines.get(0).typ == com.pullenti.ner.mail.internal.MailLine.Types.FROM) {
                ...
                MailReferent _mail = MailReferent._new1573(MailKind.HEAD);
            ...
            for(int ii = i0; ii < lines.size(); ii++) {
                if (lines.get(ii).typ == com.pullenti.ner.mail.internal.MailLine.Types.HELLO) {
                    MailReferent _mail = MailReferent._new1573(MailKind.HELLO);
            ...
            if (i0 < lines.size()) {
                if (t2 != null && t2.getPrevious() == null) {
                    ...
                    MailReferent _mail = MailReferent._new1573(MailKind.BODY);
                ...
                if (t2 != null) {
                    MailReferent _mail = MailReferent._new1573(MailKind.TAIL);
