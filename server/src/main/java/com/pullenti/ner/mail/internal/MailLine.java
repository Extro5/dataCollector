...
public class MailLine extends com.pullenti.ner.MetaToken {
    ...
    public com.pullenti.ner.Referent getMailAddr() {
        for(com.pullenti.ner.Token t = getBeginToken(); t != null && t.endChar <= endChar; t = t.getNext()) {
            if (t.getReferent() != null && com.pullenti.n2j.Utils.stringsEq(t.getReferent().getTypeName(), "URI")) {
                if (com.pullenti.n2j.Utils.stringsEq(t.getReferent().getStringValue("SCHEME"), "mailto")) 
    ...
    public boolean isRealFrom() {
        com.pullenti.ner.TextToken tt = (com.pullenti.ner.TextToken)com.pullenti.n2j.Utils.cast(getBeginToken(), com.pullenti.ner.TextToken.class);
        if (tt == null) 
            return false;
        return com.pullenti.n2j.Utils.stringsEq(tt.term, "FROM") || com.pullenti.n2j.Utils.stringsEq(tt.term, "ОТ");
    }


    @Override
    public String toString() {
        return (mustBeFirstLine ? "(1) " : "") + lev + " " + typ.toString() + ": " + getSourceText();
    }
    ...
    public static MailLine parse(com.pullenti.ner.Token t0, int _lev) {
        ...
        if (res.typ == Types.UNDEFINED) {
            ...
            for(com.pullenti.ner.Token t = t0; t != null && t.endChar <= res.endChar; t = t.getNext()) {
                ...
                if (tok != null && (npt.getEndToken() instanceof com.pullenti.ner.TextToken)) {
                    String term = (((com.pullenti.ner.TextToken)com.pullenti.n2j.Utils.cast(npt.getEndToken(), com.pullenti.ner.TextToken.class))).term;
                    if (com.pullenti.n2j.Utils.stringsEq(term, "ДЕЛ")) 
                        tok = null;
                }
                if (tok == null) {
        ...
        if (res.typ == Types.UNDEFINED) {
            ...
            if (t != null) {
                ...
                if (((t.isValue("ПЕРЕСЫЛАЕМОЕ", null) || t.isValue("ПЕРЕАДРЕСОВАННОЕ", null))) && t.getNext() != null && t.getNext().isValue("СООБЩЕНИЕ", null)) {
                ...
                else if (((t.getReferent() != null && com.pullenti.n2j.Utils.stringsEq(t.getReferent().getTypeName(), "DATE"))) || ((t.isValue("IL", null) && t.getNext() != null && t.getNext().isValue("GIORNO", null))) || ((t.isValue("ON", null) && (t.getNext() instanceof com.pullenti.ner.ReferentToken) && com.pullenti.n2j.Utils.stringsEq(t.getNext().getReferent().getTypeName(), "DATE")))) {
                    boolean hasFrom = false;
                    boolean hasDate = t.getReferent() != null && com.pullenti.n2j.Utils.stringsEq(t.getReferent().getTypeName(), "DATE");
                    ...
                    for(; t != null && t.endChar <= tmax; t = t.getNext()) {
                        if (t.isValue("ОТ", null) || t.isValue("FROM", null)) 
                            hasFrom = true;
                        else if (t.getReferent() != null && ((com.pullenti.n2j.Utils.stringsEq(t.getReferent().getTypeName(), "URI") || (t.getReferent() instanceof com.pullenti.ner.person.PersonReferent)))) {
                            if (com.pullenti.n2j.Utils.stringsEq(t.getReferent().getTypeName(), "URI") && hasDate) {
                                if (br1 != null) {
                                    hasFrom = true;
                                    _next = null;
                                }
                    ...
                    for(; t != null && (t.endChar < res.endChar); t = t.getNext()) {
                        if (t.getReferent() != null && ((com.pullenti.n2j.Utils.stringsEq(t.getReferent().getTypeName(), "URI") || (t.getReferent() instanceof com.pullenti.ner.person.PersonReferent)))) 
                            hasUri = true;
    ...
    public static void initialize() {
        ...
        for(String s : new String[] {"УВАЖЕНИЕ", "ПОЧТЕНИЕ", "С УВАЖЕНИЕМ", "ПОЖЕЛАНИE", "ДЕНЬ", "ХОРОШЕГО ДНЯ", "ИСКРЕННЕ ВАШ", "УДАЧА", "СПАСИБО", "ЦЕЛОВАТЬ", "ПОВАГА", "З ПОВАГОЮ", "ПОБАЖАННЯ", "ДЕНЬ", "ЩИРО ВАШ", "ДЯКУЮ", "ЦІЛУВАТИ", "BEST REGARDS", "REGARDS", "BEST WISHES", "KIND REGARDS", "GOOD BYE", "BYE", "THANKS", "THANK YOU", "MANY THANKS", "DAY", "VERY MUCH", "HAVE", "LUCK", "Yours sincerely", "sincerely Yours", "Looking forward", "Ar cieņu"}) {
            m_RegardWords.add(new com.pullenti.ner.core.Termin(s.toUpperCase(), new com.pullenti.morph.MorphLang(null), false));
        }
        ...
        for(String s : new String[] {"FROM", "TO", "CC", "SENT", "SUBJECT", "SENDER", "TIME", "ОТ КОГО", "КОМУ", "ДАТА", "ТЕМА", "КОПИЯ", "ОТ", "ОТПРАВЛЕНО", "WHEN", "WHERE"}) {
            m_FromWords.add(new com.pullenti.ner.core.Termin(s, new com.pullenti.morph.MorphLang(null), false));
        }
        ...
        for(String s : new String[] {"HI", "HELLO", "DEAR", "GOOD MORNING", "GOOD DAY", "GOOD EVENING", "GOOD NIGHT", "ЗДРАВСТВУЙ", "ЗДРАВСТВУЙТЕ", "ПРИВЕТСТВУЮ", "ПРИВЕТ", "ПРИВЕТИК", "УВАЖАЕМЫЙ", "ДОРОГОЙ", "ЛЮБЕЗНЫЙ", "ДОБРОЕ УТРО", "ДОБРЫЙ ДЕНЬ", "ДОБРЫЙ ВЕЧЕР", "ДОБРОЙ НОЧИ", "ЗДРАСТУЙ", "ЗДРАСТУЙТЕ", "ВІТАЮ", "ПРИВІТ", "ПРИВІТ", "ШАНОВНИЙ", "ДОРОГИЙ", "ЛЮБИЙ", "ДОБРОГО РАНКУ", "ДОБРИЙ ДЕНЬ", "ДОБРИЙ ВЕЧІР", "ДОБРОЇ НОЧІ"}) {
            m_HelloWords.add(new com.pullenti.ner.core.Termin(s, new com.pullenti.morph.MorphLang(null), false));
        }
    }
