...
public class TransportAnalyzer extends com.pullenti.ner.Analyzer {
    ...
    public java.util.HashMap<String, byte[]> getImages() {
        ...
        res.put(TransportKind.FLY.toString(), com.pullenti.ner.business.internal.ResourceHelper.getBytes("fly.png"));
        res.put(TransportKind.SHIP.toString(), com.pullenti.ner.business.internal.ResourceHelper.getBytes("ship.png"));
        res.put(TransportKind.SPACE.toString(), com.pullenti.ner.business.internal.ResourceHelper.getBytes("space.png"));
        res.put(TransportKind.TRAIN.toString(), com.pullenti.ner.business.internal.ResourceHelper.getBytes("train.png"));
        res.put(TransportKind.AUTO.toString(), com.pullenti.ner.business.internal.ResourceHelper.getBytes("auto.png"));
        res.put(com.pullenti.ner.transport.internal.MetaTransport.IMAGEID, com.pullenti.ner.business.internal.ResourceHelper.getBytes("transport.png"));
        return res;
    }
    ...
    public com.pullenti.ner.Referent createReferent(String type) {
        if (com.pullenti.n2j.Utils.stringsEq(type, TransportReferent.OBJ_TYPENAME)) 
    ...
    public void process(com.pullenti.ner.core.AnalysisKit kit) {
        ...
        java.util.HashMap<String, java.util.ArrayList<com.pullenti.ner.Referent>> objsByModel = new java.util.HashMap< >();
        ...
        for(com.pullenti.ner.Token t = kit.firstToken; t != null; t = t.getNext()) {
            ...
            if (its == null) 
                continue;
            java.util.ArrayList<com.pullenti.ner.ReferentToken> rts = tryAttach(its, false);
            if (rts != null) {
                for(com.pullenti.ner.ReferentToken rt : rts) {
                    ...
                    for(com.pullenti.ner.Token tt = t.getPrevious(); tt != null && (cou < 1000); tt = tt.getPrevious(),cou++) {
                        TransportReferent tr = (TransportReferent)com.pullenti.n2j.Utils.cast(tt.getReferent(), TransportReferent.class);
                        if (tr == null) 
                            continue;
                        boolean ok = true;
                    ...
                    t = rt;
                    for(com.pullenti.ner.Slot s : rt.referent.getSlots()) {
                        if (com.pullenti.n2j.Utils.stringsEq(s.getTypeName(), TransportReferent.ATTR_MODEL)) {
                            String mod = s.getValue().toString();
                            for(int k = 0; k < 2; k++) {
                                if (!Character.isDigit(mod.charAt(0))) {
                                    java.util.ArrayList<com.pullenti.ner.Referent> li;
                                    com.pullenti.n2j.Outargwrapper<java.util.ArrayList<com.pullenti.ner.Referent>> inoutarg2638 = new com.pullenti.n2j.Outargwrapper< >();
                                    Boolean inoutres2639 = com.pullenti.n2j.Utils.tryGetValue(objsByModel, mod, inoutarg2638);
                                    li = inoutarg2638.value;
                                    if (!inoutres2639) 
                                        objsByModel.put(mod, (li = new java.util.ArrayList< >()));
                                    if (!li.contains(rt.referent)) 
                                        li.add(rt.referent);
                                    models.addStr(mod, li, new com.pullenti.morph.MorphLang(null), false);
                                }
                                if (k > 0) 
                                    break;
                        ...
                        else if (com.pullenti.n2j.Utils.stringsEq(s.getTypeName(), TransportReferent.ATTR_NAME)) 
                            objByNames.add(com.pullenti.ner.core.Termin._new118(s.getValue().toString(), rt.referent));
                    }
                }
            }
        }
        ...
        for(com.pullenti.ner.Token t = kit.firstToken; t != null; t = t.getNext()) {
            ...
            if (br != null) {
                ...
                if (toks != null && toks.getEndToken().getNext() == br.getEndToken()) {
                    com.pullenti.ner.ReferentToken rt0 = new com.pullenti.ner.ReferentToken((com.pullenti.ner.Referent)com.pullenti.n2j.Utils.cast(toks.termin.tag, com.pullenti.ner.Referent.class), br.getBeginToken(), br.getEndToken(), null);
                    kit.embedToken(rt0);
                    t = rt0;
                    continue;
                }
            ...
            java.util.ArrayList<com.pullenti.ner.Referent> li = (java.util.ArrayList<com.pullenti.ner.Referent>)com.pullenti.n2j.Utils.cast(tok.termin.tag, java.util.ArrayList.class);
            if (li != null && li.size() == 1) 
                tr = li.get(0);
            else 
                tr = (com.pullenti.ner.Referent)com.pullenti.n2j.Utils.cast(tok.termin.tag, com.pullenti.ner.Referent.class);
            if (tr != null) {
    ...
    public com.pullenti.ner.ReferentToken processReferent(com.pullenti.ner.Token begin, com.pullenti.ner.Token end) {
        ...
        if (its == null) 
            return null;
        java.util.ArrayList<com.pullenti.ner.ReferentToken> rr = tryAttach(its, true);
