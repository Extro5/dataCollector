...
public class ContentAnalyzeWhapper {
    ...
    public void analyze(FragToken root, FragToken _topDoc, boolean isCitat, com.pullenti.ner.instrument.InstrumentKind rootKind) {
        topDoc = _topDoc;
        citKind = rootKind;
        java.util.ArrayList<InstrToken1> _lines = new java.util.ArrayList< >();
        int directives = 0;
        int parts = 0;
        ...
        for(com.pullenti.ner.Token t = root.getBeginToken(); t != null; t = t.getNext()) {
            ...
            InstrToken1 lt = InstrToken1.parse(t, false, _topDoc, 0, (_lines.size() > 0 ? _lines.get(_lines.size() - 1) : null), isCitat && t == root.getBeginToken(), root.getEndToken().endChar, false);
            if (lt == null) 
                continue;
            if (lt.typ == InstrToken1.Types.CLAUSE && lt.numbers.size() == 1 && com.pullenti.n2j.Utils.stringsEq(lt.numbers.get(0), "13")) {
            }
            if (lt.numTyp == NumberTypes.DIGIT && lt.numbers.size() == 1 && com.pullenti.n2j.Utils.stringsEq(lt.numbers.get(0), "10")) {
            }
        ...
        if (directives > 0 && directives > parts) 
            _analizeContentWithDirectives(root, _lines);
        else 
            _analizeContentWithContainers(root, _lines, 0, _topDoc);
        _analizePreamble(root);
        root._analizeTables();
        if (docTyp == com.pullenti.ner.decree.DecreeKind.CONTRACT) {
        }
        else 
            _correctKodexParts(root);
        _analizeSections(root);
        _correctNames(root);
    ...
    private void _analizeContentWithContainers(FragToken root, java.util.ArrayList<InstrToken1> _lines, int topLevel, FragToken _topDoc) {
        java.util.ArrayList<InstrToken1> nums = new java.util.ArrayList< >();
        int k = 0;
        int lev = 100;
        ...
        if (((root.kind == com.pullenti.ner.instrument.InstrumentKind.PARAGRAPH && _lines.size() > 10 && _lines.get(0).typ == InstrToken1.Types.LINE) && _lines.get(0).numbers.size() > 0 && !_lines.get(0).hasVerb) && _lines.get(1).typ == InstrToken1.Types.CLAUSE) {
            ...
            for(int ii = 2; ii < (_lines.size() - 1); ii++) {
                ...
                if (NumberingHelper.calcDelta(la, ch, false) != 1) 
                    continue;
                if (la.numTyp != ch.numTyp || com.pullenti.n2j.Utils.stringsNe(la.numSuffix, ch.numSuffix)) 
                    continue;
        ...
        if (koef < 2) {
            if (topLevel < InstrToken1._calcRank(InstrToken1.Types.CHAPTER)) {
                if (_analizeChapterWithoutKeywords(root, _lines, _topDoc)) 
                    return;
            }
            _analizeContentWithoutContainers(root, _lines, false, false, false);
            return;
        }
        int n = 0;
        int names = 0;
        FragToken fr = null;
        java.util.ArrayList<InstrToken1> blk = new java.util.ArrayList< >();
        for(int i = 0; i <= _lines.size(); i++) {
            InstrToken1 li = (i < _lines.size() ? _lines.get(i) : null);
            if (li == null || (((n < nums.size()) && li == nums.get(n)))) {
                if (blk.size() > 0) {
                    if (fr == null) {
                        fr = FragToken._new1206(blk.get(0).getBeginToken(), blk.get(blk.size() - 1).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT);
                        if (blk.size() == 1) 
                            fr.itok = blk.get(0);
                        root.children.add(fr);
                    }
                    fr.setEndToken(blk.get(blk.size() - 1).getEndToken());
                    _analizeContentWithContainers(fr, blk, lev, _topDoc);
                    blk.clear();
                    fr = null;
                }
            }
            ...
            if ((n < nums.size()) && li == nums.get(n)) {
                n++;
                fr = FragToken._new1207(li.getBeginToken(), li.getEndToken(), li, li.isExpired);
                root.children.add(fr);
                ...
                if (li.getBeginToken() != li.numBeginToken && li.numBeginToken != null) 
                    fr.children.add(FragToken._new1208(li.getBeginToken(), li.numBeginToken.getPrevious(), com.pullenti.ner.instrument.InstrumentKind.KEYWORD, true, li));
                ...
                if (li.numEndToken != li.getEndToken() && li.numEndToken != null) {
                    if (!li.allUpper && ((((li.hasVerb && names == 0 && li.getEndToken().isCharOf(".:"))) || li.getEndToken().isChar(':')))) 
                        fr.children.add(FragToken._new1206(li.numEndToken.getNext(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT));
                    else {
                        FragToken frName = FragToken._new1208(li.numEndToken.getNext(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME, true, li);
                        fr.children.add(frName);
                        fr.name = FragToken.getRestoredNameMT(frName);
                    ...
                    if (_lines.get(i + 1).allUpper || ((_lines.get(i + 1).getBeginToken().isChar('['))) || _lines.get(i).getEndToken().isChar('.')) {
                        ...
                        FragToken frName = FragToken._new1208(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME, true, li);
                        fr.children.add(frName);
                        fr.name = FragToken.getRestoredNameMT(frName);
                        i = correctName(fr, frName, _lines, i);
                        names++;
            ...
            if (li.typ == InstrToken1.Types.EDITIONS && blk.size() == 0 && fr != null) 
                fr.children.add(FragToken._new1206(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.EDITIONS));
            else 
                blk.add(li);
        }
    }
        ...
        if (fr.name != null && (fr.name.length() < 100)) {
            for(; (i + 1) < _lines.size(); i++) {
                ...
                while(tt instanceof com.pullenti.ner.MetaToken) {
                    tt = (((com.pullenti.ner.MetaToken)com.pullenti.n2j.Utils.cast(tt, com.pullenti.ner.MetaToken.class))).getBeginToken();
                }
                if (tt.chars.isCapitalUpper() || !tt.chars.isLetter()) {
    ...
    private boolean _analizeChapterWithoutKeywords(FragToken root, java.util.ArrayList<InstrToken1> _lines, FragToken _topDoc) {
        ...
        if (nums == null || nums.get(0).numbers.size() != 1 || com.pullenti.n2j.Utils.stringsNe(nums.get(0).numbers.get(0), "1")) {
            if (docTyp == com.pullenti.ner.decree.DecreeKind.CONTRACT) {
                java.util.ArrayList<InstrToken1> nums1 = new java.util.ArrayList< >();
                String num0 = "1";
                boolean ok = true;
                for(int i = 1; i < _lines.size(); i++) {
                    InstrToken1 li = _lines.get(i);
                    InstrToken1 li0 = _lines.get(i - 1);
                    if (li.numbers.size() == 2 && com.pullenti.n2j.Utils.stringsEq(li.numbers.get(0), num0) && com.pullenti.n2j.Utils.stringsEq(li.numbers.get(1), "1")) {
                        if (li0.numbers.size() == 0 && !li0.getBeginToken().chars.isAllLower()) {
                        }
                        else if (li0.numbers.size() == 1 && com.pullenti.n2j.Utils.stringsEq(li0.numbers.get(0), num0)) {
                        }
                        else {
                            ok = false;
                            break;
                    ...
                    if (li0.isStandardTitle || ((li0.numbers.size() == 1 && com.pullenti.n2j.Utils.stringsEq(li0.numbers.get(0), num0)))) {
                        nums1.add(li0);
                        num0 = ((Integer)(nums1.size() + 1)).toString();
                    }
                }
        ...
        FragToken fr = null;
        java.util.ArrayList<InstrToken1> blk = new java.util.ArrayList< >();
        java.util.ArrayList<FragToken> childs = new java.util.ArrayList< >();
        for(int i = 0; i <= _lines.size(); i++) {
            InstrToken1 li = (i < _lines.size() ? _lines.get(i) : null);
            if (li == null || (((n < nums.size()) && li == nums.get(n))) || ((n >= nums.size() && li.isStandardTitle))) {
                if (blk.size() > 0) {
                    if (fr == null) {
                        fr = FragToken._new1206(blk.get(0).getBeginToken(), blk.get(blk.size() - 1).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT);
                        if (blk.size() == 1) 
                    ...
                    fr.setEndToken(blk.get(blk.size() - 1).getEndToken());
                    _analizeContentWithoutContainers(fr, blk, false, false, false);
                    blk.clear();
                    fr = null;
                }
            }
            ...
            if ((n < nums.size()) && li == nums.get(n)) {
                ...
                fr = FragToken._new1214(li.getBeginToken(), li.getEndToken(), li);
                childs.add(fr);
                fr.kind = com.pullenti.ner.instrument.InstrumentKind.CHAPTER;
                NumberingHelper.createNumber(fr, li);
                if (li.numEndToken != li.getEndToken() && li.numEndToken != null) {
                    if (li.hasManySpecChars) 
                        fr.children.add(FragToken._new1215(li.numEndToken.getNext(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT, li));
                    else {
                        FragToken frName = FragToken._new1208(li.numEndToken.getNext(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME, true, li);
                        fr.children.add(frName);
                        fr.name = FragToken.getRestoredNameMT(frName);
            ...
            else if (n >= nums.size() && li.isStandardTitle) {
                fr = FragToken._new1214(li.getBeginToken(), li.getEndToken(), li);
    ...
    private void _addCommentOrEdition(FragToken fr, InstrToken1 li) {
        if (li.typ == InstrToken1.Types.COMMENT) 
            fr.children.add(FragToken._new1215(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.COMMENT, li));
        else if (li.typ == InstrToken1.Types.EDITIONS) {
            FragToken edt = FragToken._new1215(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.EDITIONS, li);
            fr.children.add(edt);
            edt.referents = new java.util.ArrayList< >();
    ...
    private void _analizeContentWithoutContainers(FragToken root, java.util.ArrayList<InstrToken1> _lines, boolean isSubitem, boolean isPreamble, boolean isKodex) {
        ...
        if (root.kind == com.pullenti.ner.instrument.InstrumentKind.CLAUSE || ((root.kind == com.pullenti.ner.instrument.InstrumentKind.CHAPTER && docTyp == com.pullenti.ner.decree.DecreeKind.CONTRACT))) {
            ...
            while(_lines.size() > 0) {
                if (_lines.get(0).typ == InstrToken1.Types.COMMENT || _lines.get(0).typ == InstrToken1.Types.EDITIONS) {
                    _addCommentOrEdition(root, _lines.get(0));
                    _lines.remove(0);
                }
                else 
                    break;
            ...
            if (_lines.get(0).numbers.size() == 0 && docTyp != com.pullenti.ner.decree.DecreeKind.CONTRACT) {
                java.util.ArrayList<FragToken> parts = new java.util.ArrayList< >();
                java.util.ArrayList<InstrToken1> tmp = new java.util.ArrayList< >();
                FragToken part = null;
                for(int ii = 0; ii < _lines.size(); ii++) {
                    ...
                    if ((ii > 0 && li.numbers.size() == 0 && li.typ != InstrToken1.Types.EDITIONS) && li.typ != InstrToken1.Types.COMMENT && part != null) {
                        if (com.pullenti.ner.core.MiscHelper.canBeStartOfSentence(li.getBeginToken())) {
                            ...
                            if (end) {
                                _analizeContentWithoutContainers(part, tmp, false, false, isKodex);
                                tmp.clear();
                                part = null;
                            }
                        }
                    ...
                    if (part == null) {
                        part = FragToken._new1222(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CLAUSEPART, parts.size() + 1);
                        parts.add(part);
                    }
                ...
                if (ok) {
                    ...
                    if (parts.size() > 1) {
                        root.children.addAll(parts);
                        return;
                    }
                    else if (parts.size() == 1) {
                        root.children.addAll(parts.get(0).children);
                        return;
                    }
                }
            }
        ...
        java.util.ArrayList<FragToken> notices = new java.util.ArrayList< >();
        for(int ii = 0; ii < _lines.size(); ii++) {
            if (_lines.get(ii).typ == InstrToken1.Types.NOTICE) {
                InstrToken1 li = _lines.get(ii);
                if (((li.numbers.size() == 1 && com.pullenti.n2j.Utils.stringsEq(li.numbers.get(0), "1"))) || ((li.numbers.size() == 0 && ii == (_lines.size() - 1)))) {
                    for(int j = ii; j < _lines.size(); j++) {
                        li = _lines.get(j);
                        FragToken not = FragToken._new1215(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NOTICE, li);
                        notices.add(not);
                        if (li.numBeginToken != null && li.getBeginToken() != li.numBeginToken) 
                            not.children.add(FragToken._new1224(li.getBeginToken(), li.numBeginToken.getPrevious(), com.pullenti.ner.instrument.InstrumentKind.KEYWORD, true));
                        if (li.numbers.size() > 0) 
                            NumberingHelper.createNumber(not, li);
                        if (not.children.size() > 0) 
                            not.children.add(FragToken._new1215((com.pullenti.ner.Token)com.pullenti.n2j.Utils.notnull(li.numEndToken, li.getBeginToken()), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT, li));
                    }
                }
                for(int indRemoveRange = ii + _lines.size() - ii - 1, indMinIndex = ii; indRemoveRange >= indMinIndex; indRemoveRange--) _lines.remove(indRemoveRange);
                break;
            }
        }
        ...
        if (isKodex && nums != null) {
            ...
            for(InstrToken1 nu : nums) {
                if (nu.numSuffix != null && com.pullenti.n2j.Utils.stringsNe(nu.numSuffix, ")") && com.pullenti.n2j.Utils.stringsNe(nu.numSuffix, ".")) 
                    errCou++;
            }
            if (errCou > 0) {
        ...
        if (nums == null) {
            ...
            for(InstrToken1 li : _lines) {
                if (li.typ == InstrToken1.Types.COMMENT || li.typ == InstrToken1.Types.EDITIONS) {
                    _addCommentOrEdition(root, li);
                    last = null;
                    continue;
                }
                if (li.typ == InstrToken1.Types.INDEX) {
                    root.children.add(FragToken._new1215(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.INDEX, li));
                    last = null;
                    continue;
                }
                if (last != null && last.kind == com.pullenti.ner.instrument.InstrumentKind.CONTENT) 
                    ...
                    root.children.add((last = FragToken._new1215(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT, li)));
            }
            if (!isPreamble) {
        ...
        if (blk.size() > 0) 
            _analizeContentWithoutContainers(root, blk, false, true, isKodex);
        for(; i < _lines.size(); i++) {
            InstrToken1 li = _lines.get(i);
            int j;
            blk.clear();
            ...
            for(j = i + 1; j < _lines.size(); j++) {
                if ((n < nums.size()) && _lines.get(j) == nums.get(n)) 
                    break;
                else if (n >= nums.size() && _lines.get(j).isStandardTitle && _lines.get(j).allUpper) 
                    break;
                else 
            ...
            if (li.numEndToken != li.getEndToken() && li.numEndToken != null) 
                fr.children.add(FragToken._new1215(li.numEndToken.getNext(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT, li));
            else if (li.isStandardTitle && li.allUpper) {
                fr.kind = com.pullenti.ner.instrument.InstrumentKind.TAIL;
                fr.children.add(FragToken._new1230(li.getBeginToken(), li.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME, true));
            }
            if (blk.size() > 0) {
                fr.setEndToken(blk.get(blk.size() - 1).getEndToken());
                _analizeContentWithoutContainers(fr, blk, true, false, isKodex);
            }
            i = j - 1;
        }
    ...
    private static java.util.ArrayList<InstrToken1> _extractDirectiveSequence(java.util.ArrayList<InstrToken1> _lines) {
        java.util.ArrayList<InstrToken1> res = new java.util.ArrayList< >();
        for(int i = 0; i < _lines.size(); i++) {
     ...
     * @param proc 
     */
    private void _analizeContentWithDirectives(FragToken root, java.util.ArrayList<InstrToken1> _lines) {
        ...
        for(int i = 0; i < _lines.size(); i++) {
            if (_lines.get(i) == dirSeq.get(n)) {
                ...
                FragToken fr = _createDirectivePart(blk);
                if (fr != null) 
                    parts.add(fr);
                i = j - 1;
                n++;
        ...
        if (parts.size() > 2) {
            if (parts.get(0).name == null && parts.get(parts.size() - 1).name != null) {
                ...
                if (parts.get(0).children.size() == 0) 
                    parts.get(0).children.add(FragToken._new1215(parts.get(0).getBeginToken(), parts.get(0).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT, parts.get(0).itok));
            }
        ...
        for(FragToken p : parts) {
            if (p.children.size() > 0) 
                root.children.addAll(p.children);
            else 
                root.children.add(p);
        }
    }
    ...
    private FragToken _createDirectivePart(java.util.ArrayList<InstrToken1> _lines) {
        FragToken res = FragToken._new1206(_lines.get(0).getBeginToken(), _lines.get(_lines.size() - 1).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.DOCPART);
        java.util.ArrayList<InstrToken1> head = new java.util.ArrayList< >();
        int i;
        for(i = 0; i < _lines.size(); i++) {
            if (_lines.get(i).typ == InstrToken1.Types.DIRECTIVE) 
                break;
        ...
        if (i >= _lines.size()) {
            _analizeContentWithoutContainers(res, _lines, false, false, false);
            return res;
        }
        if (head.size() > 0) {
            FragToken frHead = FragToken._new1206(head.get(0).getBeginToken(), head.get(head.size() - 1).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT);
            _analizeContentWithoutContainers(frHead, head, false, false, false);
        ...
        res.children.add(FragToken._new1234(_lines.get(i).getBeginToken(), _lines.get(i).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.DIRECTIVE, _lines.get(i).value, _lines.get(i)));
        String vvv = _lines.get(i).value;
        if (com.pullenti.n2j.Utils.stringsEq(vvv, "УСТАНОВЛЕНИЕ") || com.pullenti.n2j.Utils.stringsEq(vvv, "ВСТАНОВЛЕННЯ")) 
            res.name = "МОТИВИРОВОЧНАЯ";
        else if (((((com.pullenti.n2j.Utils.stringsEq(vvv, "ПОСТАНОВЛЕНИЕ") || com.pullenti.n2j.Utils.stringsEq(vvv, "ОПРЕДЕЛЕНИЕ") || com.pullenti.n2j.Utils.stringsEq(vvv, "ПРИГОВОР")) || com.pullenti.n2j.Utils.stringsEq(vvv, "ПРИКАЗ") || com.pullenti.n2j.Utils.stringsEq(vvv, "РЕШЕНИЕ")) || com.pullenti.n2j.Utils.stringsEq(vvv, "ПОСТАНОВА") || com.pullenti.n2j.Utils.stringsEq(vvv, "ВИЗНАЧЕННЯ")) || com.pullenti.n2j.Utils.stringsEq(vvv, "ВИРОК") || com.pullenti.n2j.Utils.stringsEq(vvv, "НАКАЗ")) || com.pullenti.n2j.Utils.stringsEq(vvv, "РІШЕННЯ")) 
            res.name = "РЕЗОЛЮТИВНАЯ";
        for(int indRemoveRange = 0 + i + 1 - 1, indMinIndex = 0; indRemoveRange >= indMinIndex; indRemoveRange--) _lines.remove(indRemoveRange);
        if (_lines.size() > 0) 
            _analizeContentWithoutContainers(res, _lines, false, false, false);
        return res;
    }
    ...
    private void _analizeSections(FragToken root) {
        for(int k = 0; k < 2; k++) {
            java.util.ArrayList<Integer> secs = new java.util.ArrayList< >();
            java.util.ArrayList<Integer> items = new java.util.ArrayList< >();
            for(FragToken ch : root.children) {
                if (ch.kind == com.pullenti.ner.instrument.InstrumentKind.CHAPTER || ch.kind == com.pullenti.ner.instrument.InstrumentKind.CLAUSE) {
                    if (ch.number == 0 || (ch.children.size() < 2)) 
                        return;
                    java.util.ArrayList<FragToken> newChilds = new java.util.ArrayList< >();
                    int i = 0;
                    ...
                    if (ch.children.get(i).kind != com.pullenti.ner.instrument.InstrumentKind.CONTENT) {
                        ...
                        sect = FragToken._new1224(ch.children.get(i).getBeginToken(), ch.children.get(i).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.SECTION, true);
                        sect.name = (String)com.pullenti.n2j.Utils.cast(sect.value, String.class);
                        sect.value = null;
                        sect.children.add(FragToken._new1206(sect.getBeginToken(), sect.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME));
                        newChilds.add(sect);
                        ...
                        for(com.pullenti.ner.Token tt = la.getEndToken(); tt != null && tt.beginChar > la.beginChar; tt = tt.getPrevious()) {
                            if (tt.isNewlineBefore()) {
                                if (tt.chars.isCyrillicLetter() && tt.chars.isAllLower()) 
                                    continue;
                                InstrToken1 it = InstrToken1.parse(tt, true, null, 0, null, false, 0, false);
                                ...
                                if ((tt.getPrevious().endChar - la.beginChar) < 20) 
                                    break;
                                nextSect = FragToken._new1224(tt, la.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.SECTION, true);
                                nextSect.name = (String)com.pullenti.n2j.Utils.cast(nextSect.value, String.class);
                                nextSect.value = null;
                                nextSect.children.add(FragToken._new1206(tt, la.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME));
                                break;
                            }
                        }
                        if (nextSect == null) 
    ...
    private void _correctNames(FragToken root) {
        int i;
        ...
        for(i = 0; i < root.children.size(); i++) {
            ...
            for(j = 0; j < ch.children.size(); j++) {
                ...
                if (chh.kind == com.pullenti.ner.instrument.InstrumentKind.CONTENT || chh.kind == com.pullenti.ner.instrument.InstrumentKind.INDENTION || ((chh.kind == com.pullenti.ner.instrument.InstrumentKind.CLAUSEPART && chh.children.size() == 1))) {
                    if (chh.itok == null) 
                        chh.itok = InstrToken1.parse(chh.getBeginToken(), true, null, 0, null, false, 0, false);
            ...
            if (frNams == null) {
                frNams = new java.util.ArrayList< >();
                frNams.add(ch);
            }
            else {
        ...
        if (frNams != null) {
            for(FragToken ch : frNams) {
                            ...
                            if (ch.children.get(jj).kind == nam.kind) {
                                ...
                                if (empty) {
                                    if (ch.children.get(jj).kind == com.pullenti.ner.instrument.InstrumentKind.INDENTION || ch.children.size() == 0) {
                                        ...
                                        ch.children.addAll(jj, ch0.children);
                                    }
                                    break;
                                }
                            }
                ...
                nam.setDefVal2(true);
                nam.children.clear();
                ch.name = (String)com.pullenti.n2j.Utils.cast(nam.value, String.class);
            }
        }
        for(FragToken ch : root.children) {
            _correctNames(ch);
    ...
    private void _correctKodexParts(FragToken root) {
        ...
        for(FragToken ch : root.children) {
            _correctKodexParts(ch);
        }
    }

    private static java.util.ArrayList<FragToken> _splitContentByIndents(FragToken fr, int num) {
        if (fr.kind != com.pullenti.ner.instrument.InstrumentKind.CONTENT && fr.kind != com.pullenti.ner.instrument.InstrumentKind.LISTITEM) {
            ...
            if (fr.getBeginToken().isValue("АБЗАЦ", null)) {
                ...
                if (!((t instanceof com.pullenti.ner.NumberToken)) || (((com.pullenti.ner.NumberToken)com.pullenti.n2j.Utils.cast(t, com.pullenti.ner.NumberToken.class))).value != num) 
                    return null;
                int next = num;
                t = t.getNext();
                if (t != null && t.isHiphen() && (t.getNext() instanceof com.pullenti.ner.NumberToken)) {
                    next = (int)(((com.pullenti.ner.NumberToken)com.pullenti.n2j.Utils.cast(t.getNext(), com.pullenti.ner.NumberToken.class))).value;
                    t = t.getNext().getNext();
                    if (next <= num) 
                        return null;
                }
                if ((t == null || !t.isValue("УТРАТИТЬ", "ВТРАТИТИ") || t.getNext() == null) || !t.getNext().isValue("СИЛА", "ЧИННІСТЬ")) 
                    return null;
                java.util.ArrayList<FragToken> res0 = new java.util.ArrayList< >();
                for(int i = num; i <= next; i++) {
                    res0.add(FragToken._new1239(fr.getBeginToken(), fr.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.INDENTION, i, true, fr.referents));
                }
                return res0;
            }
        ...
        if (fr.children.size() > 0) 
            return null;
        if (fr.itok == null) 
            fr.itok = InstrToken1.parse(fr.getBeginToken(), true, null, 0, null, false, 0, false);
        java.util.ArrayList<FragToken> res = new java.util.ArrayList< >();
        ...
        for(com.pullenti.ner.Token tt = t0; tt != null && tt.endChar <= fr.endChar; tt = tt.getNext()) {
            if (tt.endChar == fr.endChar) {
                ...
                continue;
            FragToken re = FragToken._new1222(t0, tt, com.pullenti.ner.instrument.InstrumentKind.INDENTION, num);
            num++;
            if (t0 == fr.getBeginToken() && tt == fr.getEndToken()) 
                re.itok = fr.itok;
            if (re.itok == null) 
                re.itok = InstrToken1.parse(t0, true, null, 0, null, false, 0, false);
            if (res.size() > 100) 
                return null;
            res.add(re);
            t0 = tt.getNext();
    ...
    private void _analizePreamble(FragToken root) {
        ...
        if ((root.children.size() > 1 && root.children.get(0).kind == com.pullenti.ner.instrument.InstrumentKind.CONTENT && root.children.get(1).number > 0) && root.children.get(0).children.size() > 0) {
            ...
            if (i >= root.children.get(0).children.size()) {
                ...
                root.children.addAll(0, chh.children);
            }
        }
        for(i = 0; i < root.children.size(); i++) {
            ch = root.children.get(i);
            if (ch.kind == com.pullenti.ner.instrument.InstrumentKind.EDITIONS || ch.kind == com.pullenti.ner.instrument.InstrumentKind.COMMENT) 
                continue;
                ...
                for(com.pullenti.ner.Token t = ch.getBeginToken().getNext(); t != null && (t.endChar < ch.endChar); t = t.getNext()) {
                    if (t.isNewlineBefore()) {
                        if (t.getPrevious().isCharOf(".:;") && t.getPrevious().getPrevious() != null && ((t.getPrevious().getPrevious().isValue("НИЖЕСЛЕДУЮЩИЙ", null) || t.getPrevious().getPrevious().isValue("ДОГОВОР", null)))) {
                            InstrToken1 itt1 = InstrToken1.parse(t, true, null, 0, null, false, 0, false);
                            if (itt1 != null && !itt1.hasVerb && (itt1.endChar < ch.endChar)) {
                                FragToken clau = FragToken._new1206(t, ch.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CHAPTER);
                                ...
                                FragToken nam = FragToken._new1208(t, itt1.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.NAME, true, itt1);
                                clau.children.add(nam);
                                clau.name = FragToken.getRestoredNameMT(nam);
                                clau.children.add(FragToken._new1206(itt1.getEndToken().getNext(), ch.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CONTENT));
        ...
        if (cntCou == 1) {
            ...
            FragToken prm = FragToken._new1206(root.children.get(0).getBeginToken(), root.children.get(i - 1).getEndToken(), com.pullenti.ner.instrument.InstrumentKind.PREAMBLE);
            for(int j = 0; j < i; j++) {
                prm.children.add(root.children.get(j));
            }
            for(int indRemoveRange = 0 + i - 1, indMinIndex = 0; indRemoveRange >= indMinIndex; indRemoveRange--) root.children.remove(indRemoveRange);
