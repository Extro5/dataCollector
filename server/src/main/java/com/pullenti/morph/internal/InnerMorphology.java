...
public class InnerMorphology {
    ...
    public static void setEngines(MorphEngine engine) {
        if (engine != null) 
            m_EngineRu = (m_EngineEn = (m_EngineUa = (m_EngineBy = engine)));
    }

    private static MorphEngine m_EngineRu = new MorphEngine();

    private static MorphEngine m_EngineEn = new MorphEngine();

    private static MorphEngine m_EngineUa = new MorphEngine();

    private static MorphEngine m_EngineBy = new MorphEngine();

    private static MorphEngine m_EngineKz = new MorphEngine();
    ...
    private void onProgress(int val, int max, com.pullenti.n2j.ProgressEventHandler progress) {
        long p = (long)val;
        if (max > 0xFFFF) 
            p = p / ((long)((max / 100)));
        else 
            p = (p * ((long)100)) / ((long)max);
        if (p != lastPercent && progress != null) 
            progress.call(null, new com.pullenti.n2j.ProgressEventArgs((int)p, null));
        lastPercent = p;
    }
    ...
    public static class UniLexWrap {
        ...
        public static UniLexWrap _new7(com.pullenti.morph.MorphLang _arg1) {
     ...
     */
    public java.util.ArrayList<com.pullenti.morph.MorphToken> run(String text, boolean onlyTokenizing, com.pullenti.morph.MorphLang dlang, com.pullenti.n2j.ProgressEventHandler progress, boolean goodText) {
        ...
        java.util.ArrayList<com.pullenti.morph.MorphToken> res = new java.util.ArrayList< >(text.length() / 6);
        java.util.HashMap<String, UniLexWrap> uniLex = new java.util.HashMap< >();
        int i;
        int j;
        String term0 = null;
        int pureRusWords = 0;
        ...
        for(i = 0; i < twr.length; i++) {
            ...
            if (com.pullenti.n2j.Utils.isNullOrEmpty(term)) {
                i = j - 1;
                continue;
            }
            ...
            if (ty == 1 && !onlyTokenizing) {
                com.pullenti.n2j.Outargwrapper<UniLexWrap> inoutarg8 = new com.pullenti.n2j.Outargwrapper< >();
                Boolean inoutres9 = com.pullenti.n2j.Utils.tryGetValue(uniLex, term, inoutarg8);
                lemmas = inoutarg8.value;
                if (!inoutres9) {
                    lemmas = UniLexWrap._new7(lang);
        ...
        if (((defLang.isUndefined() || defLang.isUa())) && totRusWords > 0) {
            if (((totUkrWords > totRusWords && m_EngineUa.language.isUa())) || ((totByWords > totRusWords && m_EngineBy.language.isBy())) || ((totKzWords > totRusWords && m_EngineKz.language.isKz()))) {
                ...
                for(java.util.Map.Entry<String, UniLexWrap> kp : uniLex.entrySet()) {
                    ...
                    com.pullenti.n2j.Outargwrapper<com.pullenti.morph.MorphLang> inoutarg10 = new com.pullenti.n2j.Outargwrapper< >(lang);
                    kp.getValue().wordForms = processOneWord(kp.getKey(), inoutarg10);
                    lang = inoutarg10.value;
        ...
        for(java.util.Map.Entry<String, UniLexWrap> kp : uniLex.entrySet()) {
            ...
            com.pullenti.n2j.Outargwrapper<com.pullenti.morph.MorphLang> inoutarg11 = new com.pullenti.n2j.Outargwrapper< >(lang);
            kp.getValue().wordForms = processOneWord(kp.getKey(), inoutarg11);
            lang = inoutarg11.value;
            kp.getValue().lang = lang;
            if (com.pullenti.morph.MorphLang.ooNoteq((com.pullenti.morph.MorphLang.ooBitand(lang, com.pullenti.morph.MorphLang.RU)), com.pullenti.morph.MorphLang.UNKNOWN)) 
                totRusWords++;
            if (com.pullenti.morph.MorphLang.ooNoteq((com.pullenti.morph.MorphLang.ooBitand(lang, com.pullenti.morph.MorphLang.UA)), com.pullenti.morph.MorphLang.UNKNOWN)) 
            ...
            if (com.pullenti.morph.MorphLang.ooNoteq((com.pullenti.morph.MorphLang.ooBitand(lang, com.pullenti.morph.MorphLang.KZ)), com.pullenti.morph.MorphLang.UNKNOWN)) 
                totKzWords++;
            if (progress != null) 
                onProgress(cou, uniLex.size(), progress);
            cou++;
        }
        ...
        for(com.pullenti.morph.MorphToken r : res) {
            UniLexWrap uni = (UniLexWrap)com.pullenti.n2j.Utils.cast(r.tag, UniLexWrap.class);
            r.tag = null;
            ...
            for(i = 0; i < (res.size() - 2); i++) {
                ...
                if (ui1.isQuot()) {
                    ...
                    if ((p >= 2 && "БбТт".indexOf(text.charAt(p - 1)) >= 0 && ((p + 3) < text.length())) && "ЕеЯяЁё".indexOf(text.charAt(p + 1)) >= 0) {
                        ...
                        java.util.ArrayList<com.pullenti.morph.MorphWordForm> li = processOneWord0(wstr);
                        if (li != null && li.size() > 0 && li.get(0).isInDictionary()) {
                            res.get(i).endChar = res.get(i + 2).endChar;
                            res.get(i).term = wstr;
                            res.get(i).wordForms = li;
                            for(int indRemoveRange = i + 1 + 2 - 1, indMinIndex = i + 1; indRemoveRange >= indMinIndex; indRemoveRange--) res.remove(indRemoveRange);
                        }
                    }
                    ...
                    if (src.length() > 2) {
                        ...
                        if (li != null && li.size() > 0 && li.get(0).isInDictionary()) {
                            ...
                            for(int indRemoveRange = i0 + 1 + i1 - i0 - 1, indMinIndex = i0 + 1; indRemoveRange >= indMinIndex; indRemoveRange--) res.remove(indRemoveRange);
                        }
                    }
                }
                    ...
                    for(j = res.get(i + 1).endChar + 1; j < res.get(i + 2).beginChar; j++) {
                        ...
                        else if (!com.pullenti.n2j.Utils.isWhitespace(text.charAt(j))) 
                            break;
                        else 
                            sps++;
                    }
                    ...
                    if (!newline) {
                        if (uniLex.containsKey(fullWord) || com.pullenti.n2j.Utils.stringsEq(fullWord, "ИЗЗА")) 
                            newline = true;
                            ...
                            if (text.charAt(res.get(i + 1).beginChar) == '¬') {
                                java.util.ArrayList<com.pullenti.morph.MorphWordForm> li = processOneWord0(fullWord);
                                if (li != null && li.size() > 0 && li.get(0).isInDictionary()) 
                                    newline = true;
                            }
                        }
        ...
        for(i = 0; i < res.size(); i++) {
            ...
            if (mt.charInfo.isLastLower() && mt.getLength() > 2 && mt.charInfo.isCyrillicLetter()) {
                ...
                for(com.pullenti.morph.MorphWordForm wf : mt.wordForms) {
                    if (com.pullenti.n2j.Utils.stringsEq(wf.normalCase, pref) || com.pullenti.n2j.Utils.stringsEq(wf.normalFull, pref)) {
                        ok = true;
                        break;
                    }
                }
                if (!ok) {
                    ...
                    mt.wordForms.add(0, com.pullenti.morph.MorphWordForm._new12(pref, com.pullenti.morph.MorphClass.NOUN, (short)1));
                }
            }
        }
    ...
    private java.util.ArrayList<com.pullenti.morph.MorphWordForm> processOneWord0(String wstr) {
        ...
        com.pullenti.n2j.Outargwrapper<com.pullenti.morph.MorphLang> inoutarg13 = new com.pullenti.n2j.Outargwrapper< >(dl);
        java.util.ArrayList<com.pullenti.morph.MorphWordForm> inoutres14 = processOneWord(wstr, inoutarg13);
        dl = inoutarg13.value;
        return inoutres14;
    }

    private java.util.ArrayList<com.pullenti.morph.MorphWordForm> processOneWord(String wstr, com.pullenti.n2j.Outargwrapper<com.pullenti.morph.MorphLang> defLang) {
