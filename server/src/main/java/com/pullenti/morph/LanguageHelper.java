...
public class LanguageHelper {
    ...
    public static boolean isLatin(String str) {
        ...
        for(int i = 0; i < str.length(); i++) {
            if (!isLatinChar(str.charAt(i))) {
                if (!com.pullenti.n2j.Utils.isWhitespace(str.charAt(i)) && str.charAt(i) != '-') 
                    return false;
            }
        }
        return true;
    ...
    public static String transliteralCorrection(String value, String prevValue, boolean always) {
        ...
        if (((pureLat > 0 || always)) && quesCyr > 0) 
            ...
            if (quesCyr > 0 && quesLat > 0) {
                if (!com.pullenti.n2j.Utils.isNullOrEmpty(prevValue)) {
    ...
    private static String[] m_Preps = new String[] {("БЕЗ;ДО;ИЗ;ИЗЗА;ОТ;У;ДЛЯ;РАДИ;ВОЗЛЕ;ПОЗАДИ;ВПЕРЕДИ;БЛИЗ;ВБЛИЗИ;ВГЛУБЬ;ВВИДУ;ВДОЛЬ;ВЗАМЕН;ВКРУГ;ВМЕСТО;" + "ВНЕ;ВНИЗУ;ВНУТРИ;ВНУТРЬ;ВОКРУГ;ВРОДЕ;ВСЛЕД;ВСЛЕДСТВИЕ;ЗАМЕСТО;ИЗНУТРИ;КАСАТЕЛЬНО;КРОМЕ;" + "МИМО;НАВРОДЕ;НАЗАД;НАКАНУНЕ;НАПОДОБИЕ;НАПРОТИВ;НАСЧЕТ;ОКОЛО;ОТНОСИТЕЛЬНО;") + "ПОВЕРХ;ПОДЛЕ;ПОМИМО;ПОПЕРЕК;ПОРЯДКА;ПОСЕРЕДИНЕ;ПОСРЕДИ;ПОСЛЕ;ПРЕВЫШЕ;ПРЕЖДЕ;ПРОТИВ;СВЕРХ;" + "СВЫШЕ;СНАРУЖИ;СРЕДИ;СУПРОТИВ", "К;БЛАГОДАРЯ;ВОПРЕКИ;НАВСТРЕЧУ;СОГЛАСНО;СООБРАЗНО;ПАРАЛЛЕЛЬНО;ПОДОБНО;СООТВЕТСТВЕННО;СОРАЗМЕРНО", "ПРО;ЧЕРЕЗ;СКВОЗЬ;СПУСТЯ", "НАД;ПЕРЕД;ПРЕД", "ПРИ", "В;НА;О;ВКЛЮЧАЯ", "МЕЖДУ", "ЗА;ПОД", "ПО", "С"};

    private static MorphCase[] m_Cases = new MorphCase[] {MorphCase.GENITIVE, MorphCase.DATIVE, MorphCase.ACCUSATIVE, MorphCase.INSTRUMENTAL, MorphCase.PREPOSITIONAL, MorphCase.ooBitor(MorphCase.ACCUSATIVE, MorphCase.PREPOSITIONAL), MorphCase.ooBitor(MorphCase.GENITIVE, MorphCase.ACCUSATIVE), MorphCase.ooBitor(MorphCase.ACCUSATIVE, MorphCase.INSTRUMENTAL), MorphCase.ooBitor(MorphCase.DATIVE, MorphCase.ooBitor(MorphCase.ACCUSATIVE, MorphCase.PREPOSITIONAL)), MorphCase.ooBitor(MorphCase.GENITIVE, MorphCase.ooBitor(MorphCase.ACCUSATIVE, MorphCase.INSTRUMENTAL))};
    ...
    public static MorphCase getCaseAfterPreposition(String prep) {
        MorphCase mc;
        com.pullenti.n2j.Outargwrapper<MorphCase> inoutarg44 = new com.pullenti.n2j.Outargwrapper< >();
        Boolean inoutres45 = com.pullenti.n2j.Utils.tryGetValue(m_PrepCases, prep, inoutarg44);
        mc = inoutarg44.value;
        if (inoutres45) 
            return mc;
        else 
    ...
    private static String[] m_PrepNormsSrc = new String[] {"БЕЗ;БЕЗО", "В;ВО", "ВОКРУГ;ВКРУГ", "ВПЕРЕДИ;ВПЕРЕД", "ВСЛЕД;ВОСЛЕД", "ИЗ;ИЗО", "К;КО", "МЕЖДУ;МЕЖ", "НАД;НАДО", "О;ОБ;ОБО", "ОТ;ОТО", "ПЕРЕД;ПРЕД;ПРЕДО;ПЕРЕДО", "ПОД;ПОДО", "ПОСЕРЕДИНЕ;ПОСРЕДИ", "ПРОМЕЖДУ;ПРОМЕЖ", "С;СО", "СРЕДИ;СРЕДЬ", "ЧЕРЕЗ;ЧРЕЗ"};

    private static java.util.HashMap<String, String> m_PrepNorms;

    public static String normalizePreposition(String prep) {
        String res;
        com.pullenti.n2j.Outargwrapper<String> inoutarg46 = new com.pullenti.n2j.Outargwrapper< >();
        Boolean inoutres47 = com.pullenti.n2j.Utils.tryGetValue(m_PrepNorms, prep, inoutarg46);
    ...
    static {
        m_PrepCases = new java.util.HashMap< >();
        for(int i = 0; i < m_Preps.length; i++) {
            for(String v : com.pullenti.n2j.Utils.split(m_Preps[i], String.valueOf(';'), false)) {
                m_PrepCases.put(v, m_Cases[i]);
            }
        }
        m_PrepNorms = new java.util.HashMap< >();
        for(String s : m_PrepNormsSrc) {
            String[] vars = com.pullenti.n2j.Utils.split(s, String.valueOf(';'), false);
