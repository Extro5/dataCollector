...
public class ServerController {
    ...
    //диплом
    //добавляет юзера и програм апи кей
    @ResponseBody
    ...
    //активность пользователя + сессия (лина и кол-во)
    @ResponseBody
    @ResponseStatus(value = HttpStatus.OK)
    ...
    //графики, запрос с внешнего модуля
    @ResponseBody
    @ResponseStatus(value = HttpStatus.OK)
    ...
    public MyChartData diagram(ModelMap map, @PathVariable String firstValue, @PathVariable String secondValue) {
        ...
        if (firstValue.equals("users") && secondValue.equals("professions")) {
            ...
            //работает неправильно, нужно другой график
            // List<DeviceInfo> deviceInfoList = diagramRepository.getAllDeviceConsideringMarketName();
//            List<String> genderList = new ArrayList<>();
//            for (ActivityNameDiagram activityNameDiagram : activityNameDiagrams) {
//                if (!genderList.contains(activityNameDiagram.getGender())) {
//                    genderList.add(activityNameDiagram.getGender());
//                    chartMap.put(activityNameDiagram.getGender(), "1");
//                } else {
//                    int count = Integer.parseInt(chartMap.get(activityNameDiagram.getGender()));
//                    chartMap.replace(activityNameDiagram.getGender(), String.valueOf(count), String.valueOf(++count));
//                }
//            }
//            System.out.println("chartMapSize " + chartMap.size());
//            for (int i = 0; i < chartMap.size(); i++) {
