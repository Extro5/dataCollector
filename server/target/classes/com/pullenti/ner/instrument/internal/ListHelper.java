...
public class ListHelper {
    ...
    public static void analyze(FragToken res) {
        ...
        if (res.children.size() == 0) {
            ...
            if (((ki == com.pullenti.ner.instrument.InstrumentKind.CHAPTER || ki == com.pullenti.ner.instrument.InstrumentKind.CLAUSE || ki == com.pullenti.ner.instrument.InstrumentKind.CONTENT) || ki == com.pullenti.ner.instrument.InstrumentKind.ITEM || ki == com.pullenti.ner.instrument.InstrumentKind.SUBITEM) || ki == com.pullenti.ner.instrument.InstrumentKind.CLAUSEPART || ki == com.pullenti.ner.instrument.InstrumentKind.INDENTION) {
                java.util.ArrayList<FragToken> tmp = new java.util.ArrayList< >();
                tmp.add(res);
                        ...
                        if (((int)listBullet) != 0 && ch.getBeginToken().isChar(listBullet)) {
                            for(com.pullenti.ner.Token tt = ch.getBeginToken().getNext(); tt != null && (tt.endChar < ch.endChar); tt = tt.getNext()) {
                                if (tt.getPrevious().isChar('.') && com.pullenti.ner.core.MiscHelper.canBeStartOfSentence(tt)) {
                                    FragToken ch2 = FragToken._new1222(tt, ch.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.INDENTION, ch.number);
        ...
        for(int i = 0; i < res.children.size(); i++) {
            ...
            if (res.children.get(i).children.size() > 0) 
                ...
                if (co > 0) {
                    ...
                    if (co > 1) 
                        for(int indRemoveRange = i + 1 + co - 1 - 1, indMinIndex = i + 1; indRemoveRange >= indMinIndex; indRemoveRange--) res.children.remove(indRemoveRange);
                    i += (co - 1);
                }
            }
        }
        for(int i = changed.size() - 1; i >= 0; i--) {
            if (changed.get(i).kind == com.pullenti.ner.instrument.InstrumentKind.CONTENT) {
                ...
                res.children.addAll(j, changed.get(i).children);
            }
        }
    }
    ...
    private static int _analizeListItems(java.util.ArrayList<FragToken> chi, int ind) {
        ...
        if (res.getHasChanges() && res.getMultilineChangesValue() != null) {
            ...
            FragToken cit = FragToken._new1206(ci.getBeginToken(), ci.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.CITATION);
            ...
            if (ci.tag instanceof com.pullenti.ner.decree.DecreeChangeReferent) {
                com.pullenti.ner.decree.DecreeChangeReferent dcr = (com.pullenti.ner.decree.DecreeChangeReferent)com.pullenti.n2j.Utils.cast(ci.tag, com.pullenti.ner.decree.DecreeChangeReferent.class);
                if (dcr.getValue() != null && dcr.getValue().getNewItems().size() > 0) {
                    ...
                    com.pullenti.ner.decree.DecreePartReferent pat = (com.pullenti.ner.decree.DecreePartReferent)com.pullenti.n2j.Utils.cast(dcr.getOwners().get(0), com.pullenti.ner.decree.DecreePartReferent.class);
                    int min = 0;
            ...
            if (sub == null || sub.children.size() == 0) {
                ...
                cit.children.addAll(sub.children);
            else 
        ...
        if (res.itok == null) 
            res.itok = InstrToken1.parse(res.getBeginToken(), true, null, 0, null, false, res.endChar, false);
        java.util.ArrayList<LineToken> lines = parseList_LineToken(res.getBeginToken(), endChar, null);
        ...
        if (res.kind == com.pullenti.ner.instrument.InstrumentKind.CONTENT) {
            for(int j = ind + 1; j < chi.size(); j++) {
                if (chi.get(j).kind == com.pullenti.ner.instrument.InstrumentKind.CONTENT) {
                    java.util.ArrayList<LineToken> lines2 = parseList_LineToken(chi.get(j).getBeginToken(), chi.get(j).endChar, lines.get(lines.size() - 1));
                ...
                if (lines.get(i).number == 1) {
                    ...
                    for(int j = i + 1; j < lines.size(); j++) {
                        if (lines.get(j).number > 0) 
                            ...
                            java.util.ArrayList<LineToken> chli = (java.util.ArrayList<LineToken>)com.pullenti.n2j.Utils.cast(lt.tag, java.util.ArrayList.class);
                            if (chli == null) 
                                lt.tag = (chli = new java.util.ArrayList< >());
        ...
        for(int i = 0; i < lines.size(); i++) {
            if (lines.get(i).isListItem) {
                ...
                if (cou > 3 && com.pullenti.n2j.Utils.stringsNe(lines.get(i0).getBeginToken().getSourceText(), lines.get(i0 + 1).getBeginToken().getSourceText()) && com.pullenti.n2j.Utils.stringsEq(lines.get(i0 + 1).getBeginToken().getSourceText(), lines.get(i0 + 2).getBeginToken().getSourceText())) {
                    String pref = lines.get(i0 + 1).getBeginToken().getSourceText();
                    ok = true;
                    for(int j = i0 + 2; j < i; j++) {
                        if (com.pullenti.n2j.Utils.stringsNe(pref, lines.get(j).getBeginToken().getSourceText())) {
                            ok = false;
                            break;
                        }
                    }
                    ...
                    for(tt = lines.get(i0).getEndToken().getPrevious(); tt != null && tt != lines.get(i0).getBeginToken(); tt = tt.getPrevious()) {
                        if (com.pullenti.n2j.Utils.stringsEq(tt.getSourceText(), pref)) {
                            ok = true;
                            break;
                        }
                    }
        ...
        for(LineToken li : lines) {
            ...
            FragToken ch = FragToken._new1222(li.getBeginToken(), li.getEndToken(), (li.isListItem ? com.pullenti.ner.instrument.InstrumentKind.LISTITEM : com.pullenti.ner.instrument.InstrumentKind.CONTENT), li.number);
            ...
            res.children.add(ch);
            java.util.ArrayList<LineToken> chli = (java.util.ArrayList<LineToken>)com.pullenti.n2j.Utils.cast(li.tag, java.util.ArrayList.class);
            if (chli != null) {
                for(LineToken lt : chli) {
                    ch.children.add(FragToken._new1206(lt.getBeginToken(), lt.getEndToken(), com.pullenti.ner.instrument.InstrumentKind.LISTITEM));
                }
                if (ch.beginChar < ch.children.get(0).beginChar) 
                    ch.children.add(0, FragToken._new1206(ch.getBeginToken(), ch.children.get(0).getBeginToken().getPrevious(), com.pullenti.ner.instrument.InstrumentKind.CONTENT));
            }
        }
        return ret;
    }
    ...
    public static class LineToken extends com.pullenti.ner.MetaToken {
        ...
        public void correctBeginToken() {
            ...
            if (getBeginToken().isHiphen() && getBeginToken().getNext() != null) 
                setBeginToken(getBeginToken().getNext());
            else if ((number > 0 && getBeginToken().getNext() != null && getBeginToken().getNext().isChar(')')) && getBeginToken().getNext().getNext() != null) 
                setBeginToken(getBeginToken().getNext().getNext());
        }
    
        @Override
        public String toString() {
            return (isListItem ? "LISTITEM" : "TEXT") + ": " + getSourceText();
        }
        public LineToken() {
            super();
        }
            ...
            if ((lines.get(i).typ == InstrToken1.Types.LINE && lines.get(i).numbers.size() == 0 && lines.get(i).getBeginToken().isValue("ПРИЛОЖЕНИЯ", "ДОДАТОК")) && lines.get(i + 1).numbers.size() > 0 && lines.get(i).getEndToken().isChar(':')) {
                ...
                for(++i; i < lines.size(); i++) {
                    if (lines.get(i).numbers.size() == 0) {{
                                if (((i + 1) < lines.size()) && lines.get(i + 1).numbers.size() == 1 && com.pullenti.n2j.Utils.stringsEq(lines.get(i + 1).numbers.get(0), ((Integer)num).toString())) {
                        ...
                        int nn;
                        com.pullenti.n2j.Outargwrapper<Integer> inoutarg1398 = new com.pullenti.n2j.Outargwrapper< >();
                        boolean inoutres1399 = com.pullenti.n2j.Utils.parseInteger(lines.get(i).numbers.get(0), inoutarg1398);
                        nn = (inoutarg1398.value != null ? inoutarg1398.value : 0);
                        if (inoutres1399) 
                            num = nn + 1;
    ...
    public static LineToken parse_LineToken(com.pullenti.ner.Token t, int maxChar, LineToken prev) {
        ...
        if (res.getBeginToken().isHiphen()) 
            ...
            if (res.getBeginToken().getLengthChar() == 1 || (res.getBeginToken() instanceof com.pullenti.ner.NumberToken)) {
                res.isListItem = true;
                if (res.getBeginToken() instanceof com.pullenti.ner.NumberToken) 
                    res.number = (int)(((com.pullenti.ner.NumberToken)com.pullenti.n2j.Utils.cast(res.getBeginToken(), com.pullenti.ner.NumberToken.class))).value;
                else if ((res.getBeginToken() instanceof com.pullenti.ner.TextToken) && res.getBeginToken().getLengthChar() == 1) {
                    String te = (((com.pullenti.ner.TextToken)com.pullenti.n2j.Utils.cast(res.getBeginToken(), com.pullenti.ner.TextToken.class))).term;
    ...
    public static java.util.ArrayList<LineToken> parseList_LineToken(com.pullenti.ner.Token t, int maxChar, LineToken prev) {
        LineToken lt = parse_LineToken(t, maxChar, prev);
        if (lt == null) 
            return null;
        java.util.ArrayList<LineToken> res = new java.util.ArrayList< >();
        res.add(lt);
        String ss = lt.toString();
        for(t = lt.getEndToken().getNext(); t != null; t = t.getNext()) {
            LineToken lt0 = parse_LineToken(t, maxChar, lt);
            if (lt0 == null) 
                break;
            res.add((lt = lt0));
            t = lt0.getEndToken();
        ...
        for(j = i; j < res.size(); j++) {
            if (!res.get(j).isListItem) {
                if (res.get(j - 1).isListItem && res.get(j).getEndToken().isChar('.')) {
                    if (com.pullenti.n2j.Utils.stringsEq(res.get(j).getBeginToken().getSourceText(), res.get(i).getBeginToken().getSourceText()) || res.get(j).getBeginToken().chars.isAllLower()) {
