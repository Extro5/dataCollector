...
public class PhoneAnalyzer extends com.pullenti.ner.Analyzer {
    ...
    public com.pullenti.ner.Referent createReferent(String type) {
        if (com.pullenti.n2j.Utils.stringsEq(type, PhoneReferent.OBJ_TYPENAME)) 
    ...
    public static class PhoneAnalizerData extends com.pullenti.ner.core.AnalyzerData {
        ...
        private java.util.HashMap<String, java.util.ArrayList<com.pullenti.ner.phone.PhoneReferent>> m_PhonesHash = new java.util.HashMap< >();
    
        @Override
        public com.pullenti.ner.Referent registerReferent(com.pullenti.ner.Referent referent) {
            com.pullenti.ner.phone.PhoneReferent _phone = (com.pullenti.ner.phone.PhoneReferent)com.pullenti.n2j.Utils.cast(referent, com.pullenti.ner.phone.PhoneReferent.class);
            if (_phone == null) 
                return null;
            ...
            com.pullenti.n2j.Outargwrapper<java.util.ArrayList<com.pullenti.ner.phone.PhoneReferent>> inoutarg2435 = new com.pullenti.n2j.Outargwrapper< >();
            Boolean inoutres2436 = com.pullenti.n2j.Utils.tryGetValue(m_PhonesHash, key, inoutarg2435);
            phLi = inoutarg2435.value;
            if (!inoutres2436) 
                m_PhonesHash.put(key, (phLi = new java.util.ArrayList< >()));
    ...
    public void process(com.pullenti.ner.core.AnalysisKit kit) {
        PhoneAnalizerData ad = (PhoneAnalizerData)com.pullenti.n2j.Utils.cast(kit.getAnalyzerData(this), PhoneAnalizerData.class);
        for(com.pullenti.ner.Token t = kit.firstToken; t != null; t = t.getNext()) {
            ...
            for(com.pullenti.ner.Token tt = t.getPrevious(); tt != null; tt = tt.getPrevious()) {
                if (tt.getReferent() instanceof PhoneReferent) {
                    prevPhone = (PhoneReferent)com.pullenti.n2j.Utils.cast(tt.getReferent(), PhoneReferent.class);
                    break;
                }
                else if (tt.isChar(')')) {
            ...
            if (prevPhone != null) 
                isPhoneBefore = true;
            java.util.ArrayList<com.pullenti.ner.ReferentToken> rts = tryAttach(pli, j, isPhoneBefore, prevPhone);
            if (rts == null) {
                for(j = 1; j < pli.size(); j++) {
                    if (pli.get(j).itemType == com.pullenti.ner.phone.internal.PhoneItemToken.PhoneItemType.PREFIX) {
                        for(int indRemoveRange = 0 + j - 1, indMinIndex = 0; indRemoveRange >= indMinIndex; indRemoveRange--) pli.remove(indRemoveRange);
                        rts = tryAttach(pli, 1, true, prevPhone);
                        break;
                    }
                }
            }
            if (rts == null) 
                ...
                if ((ki == PhoneKind.UNDEFINED && prevPhone != null && !isPref) && prevPhone.getKind() != PhoneKind.MOBILE) 
                    ki = prevPhone.getKind();
                for(com.pullenti.ner.ReferentToken rt : rts) {
                    PhoneReferent ph = (PhoneReferent)com.pullenti.n2j.Utils.cast(rt.referent, PhoneReferent.class);
                    if (ki != PhoneKind.UNDEFINED) 
                        ph.setKind(ki);
                    else {
                        if (rt == rts.get(0) && (rt.getWhitespacesBeforeCount() < 3)) {
                            ...
                            if ((tt1 instanceof com.pullenti.ner.TextToken) && ((tt1.isNewlineBefore() || ((tt1.getPrevious() != null && tt1.getPrevious().isTableControlChar()))))) {
                                String term = (((com.pullenti.ner.TextToken)com.pullenti.n2j.Utils.cast(tt1, com.pullenti.ner.TextToken.class))).term;
                                if (com.pullenti.n2j.Utils.stringsEq(term, "T") || com.pullenti.n2j.Utils.stringsEq(term, "Т")) 
                                    rt.setBeginToken(tt1);
                                else if (com.pullenti.n2j.Utils.stringsEq(term, "Ф") || com.pullenti.n2j.Utils.stringsEq(term, "F")) {
                                    ph.setKind((ki = PhoneKind.FAX));
                                    rt.setBeginToken(tt1);
                                }
                                else if (com.pullenti.n2j.Utils.stringsEq(term, "M") || com.pullenti.n2j.Utils.stringsEq(term, "М")) {
    ...
    private java.util.ArrayList<com.pullenti.ner.ReferentToken> tryAttach(java.util.ArrayList<com.pullenti.ner.phone.internal.PhoneItemToken> pli, int ind, boolean isPhoneBefore, PhoneReferent prevPhone) {
        com.pullenti.ner.ReferentToken rt = _TryAttach_(pli, ind, isPhoneBefore, prevPhone, 0);
        if (rt == null) 
            return null;
        java.util.ArrayList<com.pullenti.ner.ReferentToken> res = new java.util.ArrayList< >();
        res.add(rt);
        for(int i = 0; i < 5; i++) {
            PhoneReferent ph0 = (PhoneReferent)com.pullenti.n2j.Utils.cast(rt.referent, PhoneReferent.class);
        ...
        if (add != null) {
            for(com.pullenti.ner.ReferentToken rr : res) {
                (((PhoneReferent)com.pullenti.n2j.Utils.cast(rr.referent, PhoneReferent.class))).setAddNumber(add.value);
            }
    ...
    public com.pullenti.ner.ReferentToken processReferent(com.pullenti.ner.Token begin, com.pullenti.ner.Token end) {
        ...
        com.pullenti.ner.ReferentToken rt = _TryAttach_(pli, i, true, null, 0);
        if (rt != null) {
    ...
    private com.pullenti.ner.ReferentToken _TryAttach_(java.util.ArrayList<com.pullenti.ner.phone.internal.PhoneItemToken> pli, int ind, boolean isPhoneBefore, PhoneReferent prevPhone, int lev) {
        ...
        if ((j < pli.size()) && pli.get(j).itemType == com.pullenti.ner.phone.internal.PhoneItemToken.PhoneItemType.COUNTRYCODE) {
            ...
            if (com.pullenti.n2j.Utils.stringsNe(countryCode, "8")) {
            ...
            if ((k < pli.size()) && pli.get(k).itemType == com.pullenti.ner.phone.internal.PhoneItemToken.PhoneItemType.DELIM) 
                k++;
            com.pullenti.ner.ReferentToken rrt = _TryAttach_(pli, k, isPhoneBefore, null, lev + 1);
            if (rrt != null) {
                countryCode = pli.get(j).value;
                j++;
            }
        ...
        if ((j < pli.size()) && pli.get(j).itemType == com.pullenti.ner.phone.internal.PhoneItemToken.PhoneItemType.DELIM) 
            j++;
        if ((com.pullenti.n2j.Utils.stringsEq(countryCode, "8") && cityCode == null && ((j + 3) < pli.size())) && pli.get(j).itemType == com.pullenti.ner.phone.internal.PhoneItemToken.PhoneItemType.NUMBER) {
        ...
        for(; j < pli.size(); j++) {
            ...
            if (pli.get(j).itemType == com.pullenti.ner.phone.internal.PhoneItemToken.PhoneItemType.DELIM) {
                ...
                else if (com.pullenti.n2j.Utils.stringsNe(pli.get(j).value, delim)) {
        ...
        if (num.length() < 7) {
            if (cityCode != null && (cityCode.length() + num.length()) > 7) {
                if (!isPhoneBefore && cityCode.length() == 3) {
                    ...
                    if (ii >= partLength.size()) {
                        if (com.pullenti.n2j.Utils.stringsEq(countryCode, "61")) {
                        }
                        else 
                            ok = false;
                    }
            ...
            else if (num.length() > 4 && prevPhone != null && com.pullenti.n2j.Utils.stringsEq(templ.toString(), prevPhone.m_Template)) 
                ok = true;
            else 
                ok = false;
        }
        if (com.pullenti.n2j.Utils.stringsEq(delim, ".") && countryCode == null && cityCode == null) 
            ok = false;
        ...
        if (ok) {
            if (std) {
            ...
            else if (prevPhone != null && prevPhone.getNumber() != null && (((prevPhone.getNumber().length() == num.length() || prevPhone.getNumber().length() == (num.length() + 3) || prevPhone.getNumber().length() == (num.length() + 4)) || com.pullenti.n2j.Utils.stringsEq(prevPhone.m_Template, templ.toString())))) {
            }
                ...
                if (((num.length() == 6 || num.length() == 7)) && (partLength.size() < 4) && j > 0) {
                    PhoneReferent nextPh = getNextPhone(pli.get(j - 1).getEndToken().getNext(), lev + 1);
                    if (nextPh != null) {
        ...
        PhoneReferent ph = new PhoneReferent();
        if (com.pullenti.n2j.Utils.stringsNe(countryCode, "8") && countryCode != null) 
        ...
        if (countryCode != null) {
            ...
            if (s != null) {
                ...
                if (num2.length() >= 10 && num2.length() <= 11) {
                    number = num2;
                    if (com.pullenti.n2j.Utils.stringsNe(s, "7")) 
                        ph.setCountryCode(s);
                }
            }
        ...
        if (number.length() > 11) {
            if ((number.length() < 14) && ((com.pullenti.n2j.Utils.stringsEq(countryCode, "1") || com.pullenti.n2j.Utils.stringsEq(countryCode, "43")))) {
            }
            else 
                return null;
        }
        ...
        if (countryCode != null && com.pullenti.n2j.Utils.stringsEq(countryCode, "7")) {
            if (number.length() != 10) 
                return null;
        }
    ...
    private PhoneReferent getNextPhone(com.pullenti.ner.Token t, int lev) {
        ...
        if (its == null) 
            return null;
        com.pullenti.ner.ReferentToken rt = _TryAttach_(its, 0, false, null, lev + 1);
        if (rt == null) 
            return null;
        return (PhoneReferent)com.pullenti.n2j.Utils.cast(rt.referent, PhoneReferent.class);
    }
