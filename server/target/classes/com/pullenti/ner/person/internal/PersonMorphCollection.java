...
public class PersonMorphCollection {
    ...
    public static class PersonMorphVariant {
        ...
        public static PersonMorphVariant _new2378(String _arg1, com.pullenti.morph.MorphGender _arg2, String _arg3) {
    ...
    public java.util.ArrayList<PersonMorphVariant> items = new java.util.ArrayList< >();
        ...
        for(int i = 0; i < (items.size() - 1); i++) {
            for(int k = 0; k < (items.size() - 1); k++) {
                if (m_Comparer.compare(items.get(k), items.get(k + 1)) > 0) {
                    ...
                    com.pullenti.n2j.Utils.putArrayValue(items, k + 1, items.get(k));
                    com.pullenti.n2j.Utils.putArrayValue(items, k, it);
                }
            }
        }
    }
    ...
    public boolean getHasLastnameStandardTail() {
        for(PersonMorphVariant it : items) {
            if (PersonItemToken.endsWithStdSurname_MorphPersonItem(it.value)) 
                return true;
        }
        return false;
    }
    ...
    public void add(String val, String shortval, com.pullenti.morph.MorphGender gen, boolean addOtherGenderVar) {
        ...
        if (gen == com.pullenti.morph.MorphGender.MASCULINE || gen == com.pullenti.morph.MorphGender.FEMINIE) {
            for(PersonMorphVariant it : items) {
                if (com.pullenti.n2j.Utils.stringsEq(it.value, val) && it.gender == gen) 
                    return;
            }
            items.add(PersonMorphVariant._new2378(val, gen, shortval));
            if (addOtherGenderVar) {
                ...
                String v = com.pullenti.morph.Morphology.getWordform(val, com.pullenti.morph.MorphBaseInfo._new211(com.pullenti.morph.MorphClass._new1511(true), g0));
                if (v != null) 
                    items.add(PersonMorphVariant._new2378(v, g0, shortval));
            }
        }
        else {
            add(val, shortval, com.pullenti.morph.MorphGender.MASCULINE, false);
            add(val, shortval, com.pullenti.morph.MorphGender.FEMINIE, false);
        }
    }
    ...
    public boolean remove(String val, com.pullenti.morph.MorphGender gen) {
        ...
        for(int i = items.size() - 1; i >= 0; i--) {
            if (val != null && com.pullenti.n2j.Utils.stringsNe(items.get(i).value, val)) 
                continue;
    ...
    private boolean containsItem(String v, com.pullenti.morph.MorphGender g) {
        for(PersonMorphVariant it : items) {
            if (com.pullenti.n2j.Utils.stringsEq(it.value, v) && it.gender == g) 
                return true;
        }
        return false;
    }
    ...
    public static boolean isEquals(PersonMorphCollection col1, PersonMorphCollection col2) {
        if (com.pullenti.n2j.Utils.stringsNe(col1.head, col2.head)) 
            return false;
    ...
    private static boolean intersect2(PersonMorphCollection col1, PersonMorphCollection col2) {
        ...
        java.util.ArrayList<String> uni = new java.util.ArrayList< >();
        for(String v : vals1) {
            if (vals2.contains(v)) {
                uni.add(v);
                continue;
