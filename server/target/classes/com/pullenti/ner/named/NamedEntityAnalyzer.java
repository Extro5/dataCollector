...
public class NamedEntityAnalyzer extends com.pullenti.ner.Analyzer {
    ...
    public java.util.HashMap<String, byte[]> getImages() {
        ...
        res.put(NamedEntityKind.MONUMENT.toString(), com.pullenti.ner.business.internal.ResourceHelper.getBytes("monument.png"));
        res.put(NamedEntityKind.PLANET.toString(), com.pullenti.ner.business.internal.ResourceHelper.getBytes("planet.png"));
        res.put(NamedEntityKind.LOCATION.toString(), com.pullenti.ner.business.internal.ResourceHelper.getBytes("location.png"));
        res.put(NamedEntityKind.BUILDING.toString(), com.pullenti.ner.business.internal.ResourceHelper.getBytes("building.png"));
        return res;
    }
    ...
    public com.pullenti.ner.Referent createReferent(String type) {
        if (com.pullenti.n2j.Utils.stringsEq(type, NamedEntityReferent.OBJ_TYPENAME)) 
    ...
    private static boolean canBeRef(NamedEntityKind ki, com.pullenti.ner.Referent re) {
        ...
        if (ki == NamedEntityKind.MONUMENT) {
            if (com.pullenti.n2j.Utils.stringsEq(re.getTypeName(), "PERSON") || com.pullenti.n2j.Utils.stringsEq(re.getTypeName(), "PERSONPROPERTY")) 
                return true;
        }
        else if (ki == NamedEntityKind.LOCATION) {
            if (re instanceof com.pullenti.ner.geo.GeoReferent) {
                com.pullenti.ner.geo.GeoReferent _geo = (com.pullenti.ner.geo.GeoReferent)com.pullenti.n2j.Utils.cast(re, com.pullenti.ner.geo.GeoReferent.class);
                if (_geo.isRegion() || _geo.isState()) 
                    return true;
            }
        }
        else if (ki == NamedEntityKind.BUILDING) {
            if (com.pullenti.n2j.Utils.stringsEq(re.getTypeName(), "ORGANIZATION")) 
                return true;
        }
        return false;
    }
    ...
    private static com.pullenti.ner.ReferentToken _tryAttach(java.util.ArrayList<com.pullenti.ner.named.internal.NamedItemToken> toks) {
        ...
        if (!ok || ki == NamedEntityKind.UNDEFINED) 
            return null;
        NamedEntityReferent nam = NamedEntityReferent._new1593(ki);
        if (typ != null) 
