...
public class RusLatAccord {
    ...
    private static java.util.ArrayList<RusLatAccord> getAccords() {
        if (m_Accords != null) 
            return m_Accords;
        m_Accords = new java.util.ArrayList< >();
    ...
    public static java.util.ArrayList<String> getVariants(String rusOrLat) {
        ...
        if (com.pullenti.n2j.Utils.isNullOrEmpty(rusOrLat)) 
            return res;
        rusOrLat = rusOrLat.toUpperCase();
        boolean isRus = com.pullenti.morph.LanguageHelper.isCyrillicChar(rusOrLat.charAt(0));
        java.util.ArrayList<java.util.ArrayList<RusLatAccord>> stack = new java.util.ArrayList< >();
        int i;
        for(i = 0; i < rusOrLat.length(); i++) {
            java.util.ArrayList<RusLatAccord> li = new java.util.ArrayList< >();
            int maxlen = 0;
            for(RusLatAccord a : getAccords()) {
                String pref = null;
        ...
        if (stack.size() == 0) 
            return res;
        java.util.ArrayList<Integer> ind = new java.util.ArrayList< >();
        ...
        while(true) {
            ...
            for(i = ind.size() - 1; i >= 0; i--) {
                if (com.pullenti.n2j.Utils.putArrayValue(ind, i, ind.get(i) + 1) < stack.get(i).size()) 
                    break;
                else 
                    com.pullenti.n2j.Utils.putArrayValue(ind, i, 0);
            }
            if (i < 0) 
                break;
        }
    ...
    public static boolean canBeEquals(String _rus, String _lat) {
        if (com.pullenti.n2j.Utils.isNullOrEmpty(_rus) || com.pullenti.n2j.Utils.isNullOrEmpty(_lat)) 
            return false;
    ...
    public static RusLatAccord _new1448(String _arg1, String _arg2, boolean _arg3) {
