...
public class TitleItemToken extends com.pullenti.ner.MetaToken {
    ...
    public static TitleItemToken tryAttach(com.pullenti.ner.Token t) {
        ...
        if (tt != null) {
            ...
            if (com.pullenti.n2j.Utils.stringsEq(tt.term, "ТЕМА")) {
                TitleItemToken tit = tryAttach(tt.getNext());
                if (tit != null && tit.typ == Types.TYP) {
                    t1 = tit.getEndToken();
                    if (t1.getNext() != null && t1.getNext().isChar(':')) 
                        t1 = t1.getNext();
                    return _new2593(t, t1, Types.TYPANDTHEME, tit.value);
                }
                if (tt.getNext() != null && tt.getNext().isChar(':')) 
                    t1 = tt.getNext();
                return new TitleItemToken(tt, t1, Types.THEME);
            }
            if (com.pullenti.n2j.Utils.stringsEq(tt.term, "ПО") || com.pullenti.n2j.Utils.stringsEq(tt.term, "НА")) {
            ...
            if (com.pullenti.n2j.Utils.stringsEq(tt.term, "ПЕРЕВОД") || com.pullenti.n2j.Utils.stringsEq(tt.term, "ПЕР")) {
                com.pullenti.ner.Token tt2 = tt.getNext();
                if (tt2 != null && tt2.isChar('.')) 
                    tt2 = tt2.getNext();
                if (tt2 instanceof com.pullenti.ner.TextToken) {
                    if (com.pullenti.n2j.Utils.stringsEq((((com.pullenti.ner.TextToken)com.pullenti.n2j.Utils.cast(tt2, com.pullenti.ner.TextToken.class))).term, "C") || com.pullenti.n2j.Utils.stringsEq((((com.pullenti.ner.TextToken)com.pullenti.n2j.Utils.cast(tt2, com.pullenti.ner.TextToken.class))).term, "С")) {
                        tt2 = tt2.getNext();
                        if (tt2 instanceof com.pullenti.ner.TextToken) 
            ...
            if (com.pullenti.n2j.Utils.stringsEq(tt.term, "СЕКЦИЯ") || com.pullenti.n2j.Utils.stringsEq(tt.term, "SECTION") || com.pullenti.n2j.Utils.stringsEq(tt.term, "СЕКЦІЯ")) {
                t1 = tt.getNext();
            ...
            if (tok != null) {
                ...
                if (ty == Types.TYP) {
                    ...
                    if (tit != null && tit.typ == Types.THEME) 
                        return _new2593(npt.getBeginToken(), tit.getEndToken(), Types.TYPANDTHEME, s);
                    if (com.pullenti.n2j.Utils.stringsEq(s, "РАБОТА") || com.pullenti.n2j.Utils.stringsEq(s, "РОБОТА") || com.pullenti.n2j.Utils.stringsEq(s, "ПРОЕКТ")) 
                        return null;
                    com.pullenti.ner.Token t1 = tok.getEndToken();
                    if (com.pullenti.n2j.Utils.stringsEq(s, "ДИССЕРТАЦИЯ") || com.pullenti.n2j.Utils.stringsEq(s, "ДИСЕРТАЦІЯ")) {
                        int err = 0;
                        for(com.pullenti.ner.Token ttt = t1.getNext(); ttt != null; ttt = ttt.getNext()) {
                            ...
                            if (rt != null && (rt.referent instanceof com.pullenti.ner.person.PersonPropertyReferent)) {
                                com.pullenti.ner.person.PersonPropertyReferent ppr = (com.pullenti.ner.person.PersonPropertyReferent)com.pullenti.n2j.Utils.cast(rt.referent, com.pullenti.ner.person.PersonPropertyReferent.class);
                                if (com.pullenti.n2j.Utils.stringsEq(ppr.getName(), "доктор наук")) {
                                    t1 = rt.getEndToken();
                                    s = "ДОКТОРСКАЯ ДИССЕРТАЦИЯ";
                                    break;
                                }
                                else if (com.pullenti.n2j.Utils.stringsEq(ppr.getName(), "кандидат наук")) {
                                    t1 = rt.getEndToken();
                                    s = "КАНДИДАТСКАЯ ДИССЕРТАЦИЯ";
                                    break;
                                }
                                else if (com.pullenti.n2j.Utils.stringsEq(ppr.getName(), "магистр")) {
                                    t1 = rt.getEndToken();
                    ...
                    return _new2593(npt.getBeginToken(), t1, ty, s);
                }
            }
        }
    ...
    private static TitleItemToken tryAttachSpeciality(com.pullenti.ner.Token t, boolean keyWordBefore) {
        ...
        return _new2593(t0, t, Types.SPECIALITY, val.toString());
    }
    ...
    public static void initialize() {
        ...
        for(String s : new String[] {"РАБОТА", "ДИССЕРТАЦИЯ", "ОТЧЕТ", "ОБЗОР", "ДИПЛОМ", "ПРОЕКТ", "СПРАВКА", "АВТОРЕФЕРАТ", "РЕФЕРАТ", "TECHNOLOGY ISSUES", "TECHNOLOGY COURSE", "УЧЕБНИК", "УЧЕБНОЕ ПОСОБИЕ"}) {
            m_Termins.add(com.pullenti.ner.core.Termin._new118(s, Types.TYP));
        }
        for(String s : new String[] {"РОБОТА", "ДИСЕРТАЦІЯ", "ЗВІТ", "ОГЛЯД", "ДИПЛОМ", "ПРОЕКТ", "ДОВІДКА", "АВТОРЕФЕРАТ", "РЕФЕРАТ"}) {
            m_Termins.add(com.pullenti.ner.core.Termin._new459(s, com.pullenti.morph.MorphLang.UA, Types.TYP));
        }
        for(String s : new String[] {"ДОПУСТИТЬ К ЗАЩИТА", "РЕКОМЕНДОВАТЬ К ЗАЩИТА", "ДОЛЖНОСТЬ", "ЦЕЛЬ РАБОТЫ", "НА ПРАВАХ РУКОПИСИ", "ПО ИЗДАНИЮ", "ПОЛУЧЕНО"}) {
            m_Termins.add(com.pullenti.ner.core.Termin._new118(s, Types.DUST));
        }
        for(String s : new String[] {"ДОПУСТИТИ ДО ЗАХИСТУ", "РЕКОМЕНДУВАТИ ДО ЗАХИСТ", "ПОСАДА", "МЕТА РОБОТИ", "НА ПРАВАХ РУКОПИСУ", "ПО ВИДАННЮ", "ОТРИМАНО"}) {
            m_Termins.add(com.pullenti.ner.core.Termin._new459(s, com.pullenti.morph.MorphLang.UA, Types.DUST));
        }
        for(String s : new String[] {"УТВЕРЖДАТЬ", "СОГЛАСЕН", "СТВЕРДЖУВАТИ", "ЗГОДЕН"}) {
            m_Termins.add(com.pullenti.ner.core.Termin._new118(s, Types.ADOPT));
        }
        for(String s : new String[] {"НАУЧНЫЙ РУКОВОДИТЕЛЬ", "РУКОВОДИТЕЛЬ РАБОТА", "НАУКОВИЙ КЕРІВНИК", "КЕРІВНИК РОБОТА"}) {
            m_Termins.add(com.pullenti.ner.core.Termin._new118(s, Types.BOSS));
        }
        for(String s : new String[] {"НАУЧНЫЙ КОНСУЛЬТАНТ", "КОНСУЛЬТАНТ", "НАУКОВИЙ КОНСУЛЬТАНТ"}) {
            m_Termins.add(com.pullenti.ner.core.Termin._new118(s, Types.CONSULTANT));
        }
        for(String s : new String[] {"РЕДАКТОР", "РЕДАКТОРСКАЯ ГРУППА", "РЕЦЕНЗЕНТ", "РЕДАКТОРСЬКА ГРУПА"}) {
            m_Termins.add(com.pullenti.ner.core.Termin._new118(s, Types.EDITOR));
        }
        for(String s : new String[] {"ОФИЦИАЛЬНЫЙ ОППОНЕНТ", "ОППОНЕНТ", "ОФІЦІЙНИЙ ОПОНЕНТ"}) {
            m_Termins.add(com.pullenti.ner.core.Termin._new118(s, Types.OPPONENT));
        }
        for(String s : new String[] {"ИСПОЛНИТЕЛЬ", "ОТВЕТСТВЕННЫЙ ИСПОЛНИТЕЛЬ", "АВТОР", "ДИПЛОМНИК", "КОЛЛЕКТТИВ ИСПОЛНИТЕЛЕЙ", "ВЫПОЛНИТЬ", "ИСПОЛНИТЬ"}) {
            m_Termins.add(com.pullenti.ner.core.Termin._new118(s, Types.WORKER));
        }
        for(String s : new String[] {"ВИКОНАВЕЦЬ", "ВІДПОВІДАЛЬНИЙ ВИКОНАВЕЦЬ", "АВТОР", "ДИПЛОМНИК", "КОЛЛЕКТТИВ ВИКОНАВЦІВ", "ВИКОНАТИ", "ВИКОНАТИ"}) {
            m_Termins.add(com.pullenti.ner.core.Termin._new459(s, com.pullenti.morph.MorphLang.UA, Types.WORKER));
        }
        for(String s : new String[] {"КЛЮЧЕВЫЕ СЛОВА", "KEYWORDS", "КЛЮЧОВІ СЛОВА"}) {
            m_Termins.add(com.pullenti.ner.core.Termin._new118(s, Types.KEYWORDS));
        }
    }
    ...
    public static TitleItemToken _new2593(com.pullenti.ner.Token _arg1, com.pullenti.ner.Token _arg2, Types _arg3, String _arg4) {
