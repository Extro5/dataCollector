...
public class BusinessAnalyzer extends com.pullenti.ner.Analyzer {
    ...
    public java.util.HashMap<String, byte[]> getImages() {
        ...
        res.put(com.pullenti.ner.business.internal.MetaBusinessFact.IMAGEID, com.pullenti.ner.business.internal.ResourceHelper.getBytes("businessfact.png"));
        res.put(com.pullenti.ner.business.internal.FundsMeta.IMAGEID, com.pullenti.ner.business.internal.ResourceHelper.getBytes("creditcards.png"));
        return res;
    }
    ...
    public com.pullenti.ner.Referent createReferent(String type) {
        if (com.pullenti.n2j.Utils.stringsEq(type, BusinessFactReferent.OBJ_TYPENAME)) 
            return new BusinessFactReferent();
        if (com.pullenti.n2j.Utils.stringsEq(type, FundsReferent.OBJ_TYPENAME)) 
    ...
    public void process(com.pullenti.ner.core.AnalysisKit kit) {
        ...
        for(com.pullenti.ner.Token t = kit.firstToken; t != null; t = t.getNext()) {
            com.pullenti.ner.ReferentToken rt = analizeFact(t);
            if (rt != null) {
                rt.referent = ad.registerReferent(rt.referent);
                kit.embedToken(rt);
                t = rt;
                java.util.ArrayList<com.pullenti.ner.ReferentToken> rts = _analizeLikelihoods(rt);
                if (rts != null) {
                    for(com.pullenti.ner.ReferentToken rt0 : rts) {
                        for(com.pullenti.ner.Slot s : rt0.referent.getSlots()) {
                            if (com.pullenti.n2j.Utils.stringsEq(s.getTypeName(), BusinessFactReferent.ATTR_WHAT) && (s.getValue() instanceof FundsReferent)) 
                                rt0.referent.uploadSlot(s, ad.registerReferent((com.pullenti.ner.Referent)com.pullenti.n2j.Utils.cast(s.getValue(), com.pullenti.ner.Referent.class)));
                        }
        ...
        if (t.getMorph()._getClass().isPersonalPronoun()) {
            ...
            if (refs != null) {
                for(com.pullenti.ner.Referent r : refs) {
                    if ((r instanceof com.pullenti.ner.person.PersonReferent) || (r instanceof com.pullenti.ner.org.OrganizationReferent) || (r instanceof FundsReferent)) 
    ...
    private com.pullenti.ner.ReferentToken _FindSecRefBefore(com.pullenti.ner.ReferentToken rt) {
        ...
        if (t == null || t.getWhitespacesAfterCount() > 2) 
            return null;
        if ((rt.getReferent() instanceof com.pullenti.ner.person.PersonReferent) && (t.getReferent() instanceof com.pullenti.ner.org.OrganizationReferent)) 
            return (com.pullenti.ner.ReferentToken)com.pullenti.n2j.Utils.cast(t, com.pullenti.ner.ReferentToken.class);
        return null;
    }
    ...
    private boolean _findSum(BusinessFactReferent bfr, com.pullenti.ner.Token t) {
        for(; t != null; t = t.getNext()) {
            ...
            if (r instanceof com.pullenti.ner.money.MoneyReferent) {
                FundsReferent fu = (FundsReferent)com.pullenti.n2j.Utils.cast(bfr.getValue(BusinessFactReferent.ATTR_WHAT), FundsReferent.class);
                if (fu != null) {
                    if (fu.getSum() == null) {
                        fu.setSum((com.pullenti.ner.money.MoneyReferent)com.pullenti.n2j.Utils.cast(r, com.pullenti.ner.money.MoneyReferent.class));
                        return true;
                    }
                }
    ...
    private com.pullenti.ner.ReferentToken _analizeGet(com.pullenti.ner.business.internal.BusinessFactItem bfi) {
        com.pullenti.ner.ReferentToken bef = _FindRefBefore(bfi.getBeginToken().getPrevious());
        if (bef == null) 
            return null;
        ...
        if ((t1.getReferent() instanceof FundsReferent) && !((bef.referent instanceof FundsReferent))) {
            FundsReferent fr = (FundsReferent)com.pullenti.n2j.Utils.cast(t1.getReferent(), FundsReferent.class);
            BusinessFactReferent bfr = BusinessFactReferent._new436(bfi.baseKind);
            bfr.setWho(bef.referent);
            com.pullenti.ner.ReferentToken bef2 = _FindSecRefBefore(bef);
            if (bef2 != null) {
                bfr.addSlot(BusinessFactReferent.ATTR_WHO, bef2.referent, false, 0);
                bef = bef2;
            }
            if (fr.getSource() == bef.referent && bef2 == null) {
                bef2 = _FindRefBefore(bef.getBeginToken().getPrevious());
                if (bef2 != null) {
                    bef = bef2;
                    bfr.setWho(bef.referent);
                }
            ...
            if (fr.getSource() == bef.referent) {
                ...
                for(com.pullenti.ner.Token tt = bef.getBeginToken().getPrevious(); tt != null; tt = tt.getPrevious()) {
                    ...
                    if (refs == null) 
                        continue;
                    for(com.pullenti.ner.Referent r : refs) {
                        if ((r instanceof com.pullenti.ner.org.OrganizationReferent) && r != bef.referent) {
                            cou = 1000;
                            fr.setSource((com.pullenti.ner.org.OrganizationReferent)com.pullenti.n2j.Utils.cast(r, com.pullenti.ner.org.OrganizationReferent.class));
                            break;
                        }
                    }
                }
            ...
            _findDate(bfr, bef.getBeginToken());
            _findSum(bfr, bef.getEndToken());
            return new com.pullenti.ner.ReferentToken(bfr, bef.getBeginToken(), t1, null);
        }
        if ((bfi.getMorph()._getClass().isNoun() && ((bfi.baseKind == BusinessFactKind.GET || bfi.baseKind == BusinessFactKind.SELL)) && (t1.getReferent() instanceof com.pullenti.ner.org.OrganizationReferent)) || (t1.getReferent() instanceof com.pullenti.ner.person.PersonReferent)) {
            if ((bef.referent instanceof FundsReferent) || (bef.referent instanceof com.pullenti.ner.org.OrganizationReferent)) {
                BusinessFactReferent bfr = BusinessFactReferent._new436(bfi.baseKind);
        ...
        if ((bef.referent instanceof com.pullenti.ner.org.OrganizationReferent) || (bef.referent instanceof com.pullenti.ner.person.PersonReferent)) {
            ...
            if ((((slav instanceof com.pullenti.ner.person.PersonReferent) || (slav instanceof com.pullenti.ner.org.OrganizationReferent))) && tt.getNext() != null && (tt.getNext().getReferent() instanceof FundsReferent)) {
                BusinessFactReferent bfr = BusinessFactReferent._new436(bfi.baseKind);
            ...
            else if (slav instanceof com.pullenti.ner.org.OrganizationReferent) {
                BusinessFactReferent bfr = BusinessFactReferent._new436(bfi.baseKind);
        ...
        if ((bef.referent instanceof FundsReferent) && (((t1.getReferent() instanceof com.pullenti.ner.org.OrganizationReferent) || (t1.getReferent() instanceof com.pullenti.ner.person.PersonReferent)))) {
            BusinessFactReferent bfr = BusinessFactReferent._new436(bfi.baseKind);
            bfr.setTyp((bfi.baseKind == BusinessFactKind.GET ? "покупка ценных бумаг" : ((bfi.baseKind == BusinessFactKind.SELL ? "продажа ценных бумаг" : "владение ценными бумагами"))));
            bfr.setWho(t1.getReferent());
            bfr.addWhat(bef.referent);
    ...
    private static com.pullenti.ner.Token _addWhosList(com.pullenti.ner.Token t1, BusinessFactReferent bfr) {
        if (t1 == null) 
            return null;
        if ((t1.getNext() != null && t1.getNext().isCommaAnd() && (t1.getNext().getNext() instanceof com.pullenti.ner.ReferentToken)) && com.pullenti.n2j.Utils.stringsEq(t1.getNext().getNext().getReferent().getTypeName(), t1.getReferent().getTypeName())) {
            java.util.ArrayList<com.pullenti.ner.Referent> li = new java.util.ArrayList< >();
            ...
            if (t1.getNext().isAnd()) 
                ...
                for(com.pullenti.ner.Token tt = t1.getNext().getNext().getNext(); tt != null; tt = tt.getNext()) {
                    ...
                    if (!((tt.getNext() instanceof com.pullenti.ner.ReferentToken))) 
                        break;
                    if (com.pullenti.n2j.Utils.stringsNe(tt.getNext().getReferent().getTypeName(), t1.getReferent().getTypeName())) 
                        break;
    ...
    private com.pullenti.ner.ReferentToken _analizeGet2(com.pullenti.ner.Token t) {
        ...
        if (tt != null && tt.isComma()) 
            tt = tt.getPrevious();
        com.pullenti.ner.ReferentToken bef = _FindRefBefore(tt);
        ...
        if ((r instanceof com.pullenti.ner.person.PersonReferent) || (r instanceof com.pullenti.ner.org.OrganizationReferent)) {
            ...
            if (slave == null && tt.getNext() != null) {
                if ((((r = tt.getNext().getReferent()))) != null) {
                    if ((r instanceof FundsReferent) || (r instanceof com.pullenti.ner.org.OrganizationReferent)) {
                        slave = (FundsReferent)com.pullenti.n2j.Utils.cast(r, FundsReferent.class);
                        te = tt.getNext();
                    }
                }
            }
        ...
        if (master != null && slave != null) {
            BusinessFactReferent bfr = BusinessFactReferent._new436(BusinessFactKind.HAVE);
            bfr.setWho(master);
            if (slave instanceof com.pullenti.ner.org.OrganizationReferent) {
                bfr.addWhat(slave);
    ...
    private com.pullenti.ner.ReferentToken _analizeHave(com.pullenti.ner.business.internal.BusinessFactItem bfi) {
        ...
        if (t != null && ((t.isValue("КОТОРЫЙ", null) || t.isValue("ЯКИЙ", null)))) 
            ...
            if (t1 == null) {
                if (bfi.isBasePassive && t != null && (((t.getReferent() instanceof com.pullenti.ner.person.PersonReferent) || (t.getReferent() instanceof com.pullenti.ner.org.OrganizationReferent)))) {
                    t1 = t;
        ...
        if (t1 != null) {
            ...
            if (tt0 != null && tt0.isChar(',')) 
                tt0 = tt0.getPrevious();
            com.pullenti.ner.ReferentToken bef = _FindRefBefore(tt0);
            if (bef == null) 
                return null;
            if (!((bef.referent instanceof com.pullenti.ner.org.OrganizationReferent))) 
                return null;
            t0 = bef.getBeginToken();
            slave = bef.referent;
        }
        else if (bfi.getEndToken().getMorphClassInDictionary().isNoun() && (t.getReferent() instanceof com.pullenti.ner.org.OrganizationReferent)) {
            slave = t.getReferent();
            t1 = t.getNext();
    ...
    private com.pullenti.ner.ReferentToken _analizeProfit(com.pullenti.ner.business.internal.BusinessFactItem bfi) {
        ...
        String typ = t1.getNormalCaseText(new com.pullenti.morph.MorphClass(null), true, com.pullenti.morph.MorphGender.UNDEFINED, false).toLowerCase();
        com.pullenti.ner.org.OrganizationReferent org = null;
        org = (com.pullenti.ner.org.OrganizationReferent)com.pullenti.n2j.Utils.cast(t1.getNext().getReferent(), com.pullenti.ner.org.OrganizationReferent.class);
        com.pullenti.ner.Token t = t1;
        if (org != null) 
            t = t.getNext();
        else {
            com.pullenti.ner.ReferentToken rt = t.kit.processReferent(com.pullenti.ner.org.OrganizationAnalyzer.ANALYZER_NAME, t.getNext());
            if (rt != null) {
                org = (com.pullenti.ner.org.OrganizationReferent)com.pullenti.n2j.Utils.cast(rt.referent, com.pullenti.ner.org.OrganizationReferent.class);
                t = rt.getEndToken();
            }
        }
        ...
        for(t = t.getNext(); t != null; t = t.getNext()) {
            ...
            if ((((t.getMorph()._getClass().isVerb() || t.isValue("ДО", null) || t.isHiphen()) || t.isValue("РАЗМЕР", null) || t.isValue("РОЗМІР", null))) && t.getNext() != null && (t.getNext().getReferent() instanceof com.pullenti.ner.money.MoneyReferent)) {
                if (sum != null) 
                    break;
                sum = (com.pullenti.ner.money.MoneyReferent)com.pullenti.n2j.Utils.cast(t.getNext().getReferent(), com.pullenti.ner.money.MoneyReferent.class);
                t1 = (t = t.getNext());
                continue;
            }
            com.pullenti.ner.Referent r = t.getReferent();
            ...
            else if ((r instanceof com.pullenti.ner.org.OrganizationReferent) && org == null) {
                org = (com.pullenti.ner.org.OrganizationReferent)com.pullenti.n2j.Utils.cast(r, com.pullenti.ner.org.OrganizationReferent.class);
                t1 = t;
            }
        }
        if (sum == null) 
        ...
        if (org == null) {
            for(com.pullenti.ner.Token tt = t0.getPrevious(); tt != null; tt = tt.getPrevious()) {
                if (tt.isChar('.')) 
                    break;
                BusinessFactReferent b0 = (BusinessFactReferent)com.pullenti.n2j.Utils.cast(tt.getReferent(), BusinessFactReferent.class);
                if (b0 != null) {
                    org = (com.pullenti.ner.org.OrganizationReferent)com.pullenti.n2j.Utils.cast(b0.getWho(), com.pullenti.ner.org.OrganizationReferent.class);
                    break;
                }
                if ((((org = (com.pullenti.ner.org.OrganizationReferent)com.pullenti.n2j.Utils.cast(tt.getReferent(), com.pullenti.ner.org.OrganizationReferent.class)))) != null) 
                    break;
            }
        }
        if (org == null) 
        ...
        if (dt != null) 
            ...
            _findDate(bfr, bfi.getBeginToken());
        return new com.pullenti.ner.ReferentToken(bfr, t0, t1, null);
    }
    ...
    private com.pullenti.ner.ReferentToken _analizeAgreement(com.pullenti.ner.business.internal.BusinessFactItem bfi) {
        ...
        for(com.pullenti.ner.Token t = bfi.getBeginToken().getPrevious(); t != null; t = t.getPrevious()) {
            ...
            if (r instanceof BusinessFactReferent) {
                BusinessFactReferent b = (BusinessFactReferent)com.pullenti.n2j.Utils.cast(r, BusinessFactReferent.class);
                if (b.getWho() != null && ((b.getWho2() != null || b.getWhom() != null))) {
                    first = b.getWho();
                    second = (com.pullenti.ner.Referent)com.pullenti.n2j.Utils.notnull(b.getWho2(), b.getWhom());
                    break;
                }
            }
            if (!((r instanceof com.pullenti.ner.org.OrganizationReferent))) 
                continue;
            if ((t.getPrevious() != null && ((t.getPrevious().isAnd() || t.getPrevious().isValue("К", null))) && t.getPrevious().getPrevious() != null) && (t.getPrevious().getPrevious().getReferent() instanceof com.pullenti.ner.org.OrganizationReferent)) {
        ...
        if (second == null) {
            for(com.pullenti.ner.Token t = bfi.getEndToken().getNext(); t != null; t = t.getNext()) {
                ...
                if (!((r instanceof com.pullenti.ner.org.OrganizationReferent))) 
                    continue;
                if ((t.getNext() != null && ((t.getNext().isAnd() || t.getNext().isValue("К", null))) && t.getNext().getNext() != null) && (t.getNext().getNext().getReferent() instanceof com.pullenti.ner.org.OrganizationReferent)) {
        ...
        if (first == null || second == null) 
            return null;
        BusinessFactReferent bf = BusinessFactReferent._new436(bfi.baseKind);
        bf.setWho(first);
        if (bfi.baseKind == BusinessFactKind.LAWSUIT) 
            bf.setWhom(second);
        else 
            bf.setWho2(second);
        _findDate(bf, bfi.getBeginToken());
        _findSum(bf, bfi.getBeginToken());
    ...
    private com.pullenti.ner.ReferentToken _analizeSubsidiary(com.pullenti.ner.business.internal.BusinessFactItem bfi) {
        com.pullenti.ner.Token t1 = bfi.getEndToken().getNext();
        if (t1 == null || !((t1.getReferent() instanceof com.pullenti.ner.org.OrganizationReferent))) 
            return null;
        com.pullenti.ner.Token t;
        com.pullenti.ner.org.OrganizationReferent org0 = null;
        for(t = bfi.getBeginToken().getPrevious(); t != null; t = t.getPrevious()) {
            ...
            if (t instanceof com.pullenti.ner.NumberToken) 
                continue;
            org0 = (com.pullenti.ner.org.OrganizationReferent)com.pullenti.n2j.Utils.cast(t.getReferent(), com.pullenti.ner.org.OrganizationReferent.class);
            if (org0 != null) 
                break;
        }
        if (org0 == null) 
    ...
    private com.pullenti.ner.ReferentToken _analizeFinance(com.pullenti.ner.business.internal.BusinessFactItem bfi) {
        ...
        for(com.pullenti.ner.Token t = bfi.getEndToken().getNext(); t != null; t = t.getNext()) {
            ...
            if (r instanceof com.pullenti.ner.org.OrganizationReferent) {
                if (whom == null) 
                    whom = (com.pullenti.ner.ReferentToken)com.pullenti.n2j.Utils.cast(t, com.pullenti.ner.ReferentToken.class);
            }
            else if (r instanceof com.pullenti.ner.money.MoneyReferent) {
                if (sum == null) 
                    sum = (com.pullenti.ner.money.MoneyReferent)com.pullenti.n2j.Utils.cast(r, com.pullenti.ner.money.MoneyReferent.class);
            }
            else if (r instanceof FundsReferent) {
                if (funds == null) 
                    funds = (FundsReferent)com.pullenti.n2j.Utils.cast(r, FundsReferent.class);
            }
        }
        if (whom == null) 
            return null;
    ...
    private java.util.ArrayList<com.pullenti.ner.ReferentToken> _analizeLikelihoods(com.pullenti.ner.ReferentToken rt) {
        BusinessFactReferent bfr0 = (BusinessFactReferent)com.pullenti.n2j.Utils.cast(rt.referent, BusinessFactReferent.class);
        if (bfr0 == null || bfr0.getWhats().size() != 1 || !((bfr0.getWhats().get(0) instanceof FundsReferent))) 
            return null;
        FundsReferent funds0 = (FundsReferent)com.pullenti.n2j.Utils.cast(bfr0.getWhats().get(0), FundsReferent.class);
        com.pullenti.ner.Token t;
        java.util.ArrayList<com.pullenti.ner.ReferentToken> whos = new java.util.ArrayList< >();
        java.util.ArrayList<FundsReferent> funds = new java.util.ArrayList< >();
        for(t = rt.getEndToken().getNext(); t != null; t = t.getNext()) {
            ...
            com.pullenti.ner.Referent r = t.getReferent();
            if ((r instanceof com.pullenti.ner.org.OrganizationReferent) || (r instanceof com.pullenti.ner.person.PersonReferent)) {
                whos.add((com.pullenti.ner.ReferentToken)com.pullenti.n2j.Utils.cast(t, com.pullenti.ner.ReferentToken.class));
                continue;
            }
            if (r instanceof FundsReferent) {
                funds0 = (FundsReferent)com.pullenti.n2j.Utils.cast(r, FundsReferent.class);
                funds.add(funds0);
                continue;
            }
            ...
            if (it == null) 
                break;
            FundsReferent fu = (FundsReferent)com.pullenti.n2j.Utils.cast(funds0.clone(), FundsReferent.class);
            fu.getOccurrence().clear();
            fu.addOccurenceOfRefTok(new com.pullenti.ner.ReferentToken(fu, it.getBeginToken(), it.getEndToken(), null));
            if (it.typ == com.pullenti.ner.business.internal.FundsItemTyp.PERCENT) 
                fu.setPercent(it.floatVal);
            else if (it.typ == com.pullenti.ner.business.internal.FundsItemTyp.COUNT) 
                fu.setCount(it.longVal);
            else if (it.typ == com.pullenti.ner.business.internal.FundsItemTyp.SUM) 
                fu.setSum((com.pullenti.ner.money.MoneyReferent)com.pullenti.n2j.Utils.cast(it.ref, com.pullenti.ner.money.MoneyReferent.class));
            else 
                break;
            funds.add(fu);
            t = it.getEndToken();
        ...
        for(int i = 0; i < whos.size(); i++) {
            BusinessFactReferent bfr = BusinessFactReferent._new447(bfr0.getKind(), bfr0.getTyp());
            bfr.setWho(whos.get(i).referent);
            bfr.addWhat(funds.get(i));
            for(com.pullenti.ner.Slot s : bfr0.getSlots()) {
                if (com.pullenti.n2j.Utils.stringsEq(s.getTypeName(), BusinessFactReferent.ATTR_MISC) || com.pullenti.n2j.Utils.stringsEq(s.getTypeName(), BusinessFactReferent.ATTR_WHEN)) 
