...
public class TitlePageAnalyzer extends com.pullenti.ner.Analyzer {
    ...
    public com.pullenti.ner.Referent createReferent(String type) {
        if (com.pullenti.n2j.Utils.stringsEq(type, TitlePageReferent.OBJ_TYPENAME)) 
    ...
    public com.pullenti.ner.ReferentToken processReferent1(com.pullenti.ner.Token begin, com.pullenti.ner.Token end) {
        ...
        com.pullenti.n2j.Outargwrapper<com.pullenti.ner.Token> inoutarg2609 = new com.pullenti.n2j.Outargwrapper< >();
        TitlePageReferent tpr = _process(begin, (end == null ? 0 : end.endChar), begin.kit, inoutarg2609);
        et = inoutarg2609.value;
        if (tpr == null) 
            return null;
    ...
    public void process(com.pullenti.ner.core.AnalysisKit kit) {
        ...
        com.pullenti.ner.Token et;
        com.pullenti.n2j.Outargwrapper<com.pullenti.ner.Token> inoutarg2610 = new com.pullenti.n2j.Outargwrapper< >();
        TitlePageReferent tpr = _process(kit.firstToken, 0, kit, inoutarg2610);
        et = inoutarg2610.value;
        if (tpr != null) 
            ad.registerReferent(tpr);
    }

    public static TitlePageReferent _process(com.pullenti.ner.Token begin, int maxCharPos, com.pullenti.ner.core.AnalysisKit kit, com.pullenti.n2j.Outargwrapper<com.pullenti.ner.Token> endToken) {
        endToken.value = begin;
        TitlePageReferent res = new TitlePageReferent(null);
        com.pullenti.ner.core.Termin term = null;
        java.util.ArrayList<com.pullenti.ner.old.internal.Line> lines = com.pullenti.ner.old.internal.Line.parse(begin, 30, 1500, maxCharPos);
        if (lines.size() < 1) 
            return null;
        int cou = lines.size();
        int minNewlinesCount = 10;
        java.util.HashMap<Integer, Integer> linesCountStat = new java.util.HashMap< >();
        for(int i = 0; i < lines.size(); i++) {
            ...
            int j;
            if (i > 0 && (((j = lines.get(i).getNewlinesBeforeCount()))) > 0) {
        ...
        if (maxCharPos > 0 && endChar > maxCharPos) 
            endChar = maxCharPos;
        java.util.ArrayList<com.pullenti.ner.titlepage.internal.TitleNameToken> names = new java.util.ArrayList< >();
        for(int i = 0; i < cou; i++) {
            if (i == 6) {
            }
        ...
        for(com.pullenti.ner.Token t = begin; t != null; t = t.getNext()) {
            ...
            if (tpt != null) {
                ...
                if (tpt.typ == com.pullenti.ner.titlepage.internal.TitleItemToken.Types.TYP) {
                    if (res.getTypes().size() == 0) 
                        ...
                        if (com.pullenti.n2j.Utils.stringsEq(ty, "РЕФЕРАТ")) 
                            res.addType(tpt.value);
                        else if (com.pullenti.n2j.Utils.stringsEq(ty, "АВТОРЕФЕРАТ")) {
                            if (com.pullenti.n2j.Utils.stringsEq(tpt.value, "КАНДИДАТСКАЯ ДИССЕРТАЦИЯ")) 
                                res.addSlot(TitlePageReferent.ATTR_TYPE, "автореферат кандидатской диссертации", true, 0);
                            else if (com.pullenti.n2j.Utils.stringsEq(tpt.value, "ДОКТОРСКАЯ ДИССЕРТАЦИЯ")) 
                                res.addSlot(TitlePageReferent.ATTR_TYPE, "автореферат докторской диссертации", true, 0);
                            else if (com.pullenti.n2j.Utils.stringsEq(tpt.value, "МАГИСТЕРСКАЯ ДИССЕРТАЦИЯ")) 
                                res.addSlot(TitlePageReferent.ATTR_TYPE, "автореферат магистерской диссертации", true, 0);
                            else if (com.pullenti.n2j.Utils.stringsEq(tpt.value, "КАНДИДАТСЬКА ДИСЕРТАЦІЯ")) 
                                res.addSlot(TitlePageReferent.ATTR_TYPE, "автореферат кандидатської дисертації", true, 0);
                            else if (com.pullenti.n2j.Utils.stringsEq(tpt.value, "ДОКТОРСЬКА ДИСЕРТАЦІЯ")) 
                                res.addSlot(TitlePageReferent.ATTR_TYPE, "автореферат докторської дисертації", true, 0);
                            else if (com.pullenti.n2j.Utils.stringsEq(tpt.value, "МАГІСТЕРСЬКА ДИСЕРТАЦІЯ")) 
                                res.addSlot(TitlePageReferent.ATTR_TYPE, "автореферат магістерської дисертації", true, 0);
                            else 
                                res.addType(tpt.value);
                        }
                        else if (com.pullenti.n2j.Utils.stringsEq(tpt.value, "РЕФЕРАТ") || com.pullenti.n2j.Utils.stringsEq(tpt.value, "АВТОРЕФЕРАТ")) {
            ...
            if (!t.isNewlineBefore() && (t.getPrevious() instanceof com.pullenti.ner.TextToken)) {
                String s = (((com.pullenti.ner.TextToken)com.pullenti.n2j.Utils.cast(t.getPrevious(), com.pullenti.ner.TextToken.class))).term;
                if (com.pullenti.n2j.Utils.stringsEq(s, "ИМЕНИ") || com.pullenti.n2j.Utils.stringsEq(s, "ИМ")) 
                    continue;
                if (com.pullenti.n2j.Utils.stringsEq(s, ".") && t.getPrevious().getPrevious() != null && t.getPrevious().getPrevious().isValue("ИМ", null)) 
                    continue;
            }
            for(com.pullenti.ner.Referent r : rli) {
                if (r instanceof com.pullenti.ner.person.PersonReferent) {
                    if (r != rli.get(0)) 
                        continue;
                    com.pullenti.ner.person.PersonReferent p = (com.pullenti.ner.person.PersonReferent)com.pullenti.n2j.Utils.cast(r, com.pullenti.ner.person.PersonReferent.class);
                    ...
                    if (typ != com.pullenti.ner.titlepage.internal.TitleItemToken.Types.UNDEFINED) {
                        ...
                        for(com.pullenti.ner.Token tt = t.getNext(); tt != null; tt = tt.getNext()) {
                            ...
                            if (rr instanceof com.pullenti.ner.person.PersonReferent) {
                                if (pr.calcTypFromAttrs((com.pullenti.ner.person.PersonReferent)com.pullenti.n2j.Utils.cast(r, com.pullenti.ner.person.PersonReferent.class)) != com.pullenti.ner.titlepage.internal.TitleItemToken.Types.UNDEFINED) 
                                    break;
                                else 
                                    continue;
                            }
                ...
                if (r instanceof com.pullenti.ner.date.DateReferent) {
                    if (res.getDate() == null) {
                        res.setDate((com.pullenti.ner.date.DateReferent)com.pullenti.n2j.Utils.cast(r, com.pullenti.ner.date.DateReferent.class));
                        if (t.endChar > endToken.value.endChar) 
                            endToken.value = t;
                    }
                }
                else if (r instanceof com.pullenti.ner.geo.GeoReferent) {
                    if (res.getCity() == null && (((com.pullenti.ner.geo.GeoReferent)com.pullenti.n2j.Utils.cast(r, com.pullenti.ner.geo.GeoReferent.class))).isCity()) {
                        res.setCity((com.pullenti.ner.geo.GeoReferent)com.pullenti.n2j.Utils.cast(r, com.pullenti.ner.geo.GeoReferent.class));
                        if (t.endChar > endToken.value.endChar) 
                            endToken.value = t;
                    }
                }
                if (r instanceof com.pullenti.ner.org.OrganizationReferent) {
                    com.pullenti.ner.org.OrganizationReferent _org = (com.pullenti.ner.org.OrganizationReferent)com.pullenti.n2j.Utils.cast(r, com.pullenti.ner.org.OrganizationReferent.class);
                    if (_org.getTypes().contains("курс") && _org.getNumber() != null) {
                        int i;
                        com.pullenti.n2j.Outargwrapper<Integer> inoutarg2611 = new com.pullenti.n2j.Outargwrapper< >();
                        boolean inoutres2612 = com.pullenti.n2j.Utils.parseInteger(_org.getNumber(), inoutarg2611);
                        i = (inoutarg2611.value != null ? inoutarg2611.value : 0);
                        if (inoutres2612 && i > 0 && (i < 8)) 
                            res.setStudentYear(i);
                    }
                    for(; _org.getHigher() != null; _org = _org.getHigher()) {
                        if (_org.getKind() != com.pullenti.ner.org.OrganizationKind.DEPARTMENT) 
                            break;
                    }
                    if (_org.getKind() != com.pullenti.ner.org.OrganizationKind.DEPARTMENT) {
                        if (res.getOrg() == null) 
                            res.setOrg(_org);
                        else if (com.pullenti.ner.org.OrganizationReferent.canBeHigher(res.getOrg(), _org)) 
                            res.setOrg(_org);
                    }
                    if (t.endChar > endToken.value.endChar) 
                        endToken.value = t;
        ...
        if (res.getValue(TitlePageReferent.ATTR_AUTHOR) == null) {
        ...
        if (res.getCity() == null && res.getOrg() != null) {
            com.pullenti.ner.Slot s = res.getOrg().findSlot(com.pullenti.ner.org.OrganizationReferent.ATTR_GEO, null, true);
            if (s != null && (s.getValue() instanceof com.pullenti.ner.geo.GeoReferent)) {
                if ((((com.pullenti.ner.geo.GeoReferent)com.pullenti.n2j.Utils.cast(s.getValue(), com.pullenti.ner.geo.GeoReferent.class))).isCity()) 
                    res.setCity((com.pullenti.ner.geo.GeoReferent)com.pullenti.n2j.Utils.cast(s.getValue(), com.pullenti.ner.geo.GeoReferent.class));
            }
        }
        if (res.getDate() == null) {
            for(com.pullenti.ner.Token t = begin; t != null && t.endChar <= endChar; t = t.getNext()) {
                com.pullenti.ner.geo.GeoReferent city = (com.pullenti.ner.geo.GeoReferent)com.pullenti.n2j.Utils.cast(t.getReferent(), com.pullenti.ner.geo.GeoReferent.class);
                if (city == null) 
                    continue;
                ...
                if (rt != null) {
                    ...
                    res.setDate((com.pullenti.ner.date.DateReferent)com.pullenti.n2j.Utils.cast(rt.referent, com.pullenti.ner.date.DateReferent.class));
                    if (kit != null) 
                        kit.embedToken(rt);
                    break;
                }
