...
public class GeoAnalyzer extends com.pullenti.ner.Analyzer {
    ...
    public java.util.HashMap<String, byte[]> getImages() {
        ...
        res.put(com.pullenti.ner.geo.internal.MetaGeo.COUNTRYCITYIMAGEID, com.pullenti.ner.address.internal.ResourceHelper.getBytes("countrycity.png"));
        res.put(com.pullenti.ner.geo.internal.MetaGeo.COUNTRYIMAGEID, com.pullenti.ner.address.internal.ResourceHelper.getBytes("country.png"));
        res.put(com.pullenti.ner.geo.internal.MetaGeo.CITYIMAGEID, com.pullenti.ner.address.internal.ResourceHelper.getBytes("city.png"));
        res.put(com.pullenti.ner.geo.internal.MetaGeo.DISTRICTIMAGEID, com.pullenti.ner.address.internal.ResourceHelper.getBytes("district.png"));
        res.put(com.pullenti.ner.geo.internal.MetaGeo.REGIONIMAGEID, com.pullenti.ner.address.internal.ResourceHelper.getBytes("region.png"));
        res.put(com.pullenti.ner.geo.internal.MetaGeo.TERRIMAGEID, com.pullenti.ner.address.internal.ResourceHelper.getBytes("territory.png"));
        res.put(com.pullenti.ner.geo.internal.MetaGeo.UNIONIMAGEID, com.pullenti.ner.address.internal.ResourceHelper.getBytes("union.png"));
        return res;
    }
    ...
    public com.pullenti.ner.Referent createReferent(String type) {
        if (com.pullenti.n2j.Utils.stringsEq(type, GeoReferent.OBJ_TYPENAME)) 
    ...
    public static class GeoAnalyzerDataWithOntology extends com.pullenti.ner.core.AnalyzerDataWithOntology {
        ...
        public com.pullenti.ner.Referent registerReferent(com.pullenti.ner.Referent referent) {
            com.pullenti.ner.geo.GeoReferent g = (com.pullenti.ner.geo.GeoReferent)com.pullenti.n2j.Utils.cast(referent, com.pullenti.ner.geo.GeoReferent.class);
            if (g != null) {
                if (g.isState()) {
                }
                    ...
                    for(com.pullenti.ner.Slot s : g.getSlots()) {
                        if (com.pullenti.n2j.Utils.stringsEq(s.getTypeName(), com.pullenti.ner.geo.GeoReferent.ATTR_NAME)) 
                            names.add((String)com.pullenti.n2j.Utils.cast(s.getValue(), String.class));
                        else if (com.pullenti.n2j.Utils.stringsEq(s.getTypeName(), com.pullenti.ner.geo.GeoReferent.ATTR_TYPE)) {
                            String typ = (String)com.pullenti.n2j.Utils.cast(s.getValue(), String.class);
                    ...
                    for(int i = 0; i < names.size(); i++) {
                        ...
                        int ii = n.indexOf(' ');
                        if (ii > 0) {
                            if (g.getValue(com.pullenti.ner.geo.GeoReferent.ATTR_REF) instanceof com.pullenti.ner.Referent) 
                                continue;
                        ...
                        for(String end : com.pullenti.ner.geo.GeoAnalyzer.ends_GeoAnalyzerDataWithOntology) {
                            if (com.pullenti.morph.LanguageHelper.endsWith(n, end)) {
                                String nn = n.substring(0, 0+(n.length() - 3));
                                for(String end2 : com.pullenti.ner.geo.GeoAnalyzer.ends_GeoAnalyzerDataWithOntology) {
                                    if (com.pullenti.n2j.Utils.stringsNe(end2, end)) {
                    ...
                    if (basNam != null && names.size() > 0 && com.pullenti.n2j.Utils.stringsNe(names.get(0), basNam)) {
    ...
    public void process(com.pullenti.ner.core.AnalysisKit kit) {
        com.pullenti.ner.core.AnalyzerDataWithOntology ad = (com.pullenti.ner.core.AnalyzerDataWithOntology)com.pullenti.n2j.Utils.cast(kit.getAnalyzerData(this), com.pullenti.ner.core.AnalyzerDataWithOntology.class);
        for(com.pullenti.ner.Token t = kit.firstToken; t != null; t = t.getNext()) {
            t.setInnerBool(false);
        }
        java.util.ArrayList<GeoReferent> nonRegistered = new java.util.ArrayList< >();
        for(int step = 0; step < 2; step++) {
            for(com.pullenti.ner.Token t = kit.firstToken; t != null; t = t.getNext()) {
                if (ad.getReferents().size() >= 2000) 
                    break;
                if (step > 0 && (t instanceof com.pullenti.ner.ReferentToken)) {
                    GeoReferent _geo = (GeoReferent)com.pullenti.n2j.Utils.cast(t.getReferent(), GeoReferent.class);
                    if (((_geo != null && t.getNext() != null && t.getNext().isChar('(')) && t.getNext().getNext() != null && _geo.canBeEquals(t.getNext().getNext().getReferent(), com.pullenti.ner.Referent.EqualType.WITHINONETEXT)) && t.getNext().getNext().getNext() != null && t.getNext().getNext().getNext().isChar(')')) {
                        com.pullenti.ner.ReferentToken rt0 = com.pullenti.ner.ReferentToken._new709(_geo, t, t.getNext().getNext().getNext(), t.getMorph());
                        kit.embedToken(rt0);
                        t = rt0;
                        continue;
                    }
                    if ((_geo != null && t.getNext() != null && t.getNext().isHiphen()) && t.getNext().getNext() != null && _geo.canBeEquals(t.getNext().getNext().getReferent(), com.pullenti.ner.Referent.EqualType.WITHINONETEXT)) {
                        com.pullenti.ner.ReferentToken rt0 = com.pullenti.ner.ReferentToken._new709(_geo, t, t.getNext().getNext(), t.getMorph());
                        kit.embedToken(rt0);
                        t = rt0;
                        continue;
                    }
                ...
                if ((rt == null && cli.size() == 1 && cli.get(0).isAdjective) && cli.get(0).ontoItem != null) {
                    ...
                    if (tt != null) {
                        ...
                        if (cli1 != null && cli1.get(0).ontoItem != null) {
                            GeoReferent g0 = (GeoReferent)com.pullenti.n2j.Utils.cast(cli.get(0).ontoItem.referent, GeoReferent.class);
                            GeoReferent g1 = (GeoReferent)com.pullenti.n2j.Utils.cast(cli1.get(0).ontoItem.referent, GeoReferent.class);
                        ...
                        if (rt == null && (((GeoReferent)com.pullenti.n2j.Utils.cast(cli.get(0).ontoItem.referent, GeoReferent.class))).isState()) {
                            if ((rt == null && tt != null && (tt.getReferent() instanceof GeoReferent)) && tt.getWhitespacesBeforeCount() == 1) {
                                GeoReferent geo2 = (GeoReferent)com.pullenti.n2j.Utils.cast(tt.getReferent(), GeoReferent.class);
                                if (com.pullenti.ner.geo.internal.GeoOwnerHelper.canBeHigher((GeoReferent)com.pullenti.n2j.Utils.cast(cli.get(0).ontoItem.referent, GeoReferent.class), geo2)) 
                                    rt = com.pullenti.ner.ReferentToken._new709(cli.get(0).ontoItem.referent.clone(), cli.get(0).getBeginToken(), cli.get(0).getEndToken(), cli.get(0).getMorph());
                            }
                            if (rt == null && step == 0) {
                                ...
                                if (npt != null && npt.endChar >= tt.beginChar) {
                                    ...
                                    com.pullenti.ner.ReferentToken rt1;
                                    if (cits != null && (((rt1 = com.pullenti.ner.geo.internal.CityAttachHelper.tryAttachCity(cits, ad, false)))) != null) {
                ...
                if (rt == null) {
                    java.util.ArrayList<com.pullenti.ner.geo.internal.CityItemToken> cits = tryParseCityListBack(t.getPrevious());
                    if (cits != null) 
                ...
                if (rt != null) {
                    GeoReferent _geo = (GeoReferent)com.pullenti.n2j.Utils.cast(rt.referent, GeoReferent.class);
                    ...
                    if (step == 0) {
                        ...
                        while(true) {
                            com.pullenti.ner.ReferentToken rr = tryAttachTerritoryBeforeCity(tt, ad);
                            if (rr == null) 
                                break;
                            _geo = (GeoReferent)com.pullenti.n2j.Utils.cast(rr.referent, GeoReferent.class);
            ...
            if (step == 0) {
                if (!onProgress(1, 4, kit)) 
                    return;
            }
            else {
                if (!onProgress(2, 4, kit)) 
                    return;
        ...
        for(com.pullenti.ner.Token t = kit.firstToken; t != null; t = (t == null ? null : t.getNext())) {
            GeoReferent g = (GeoReferent)com.pullenti.n2j.Utils.cast(t.getReferent(), GeoReferent.class);
            if (g == null) 
                continue;
            ...
            for(t = t.getNext(); t != null; t = t.getNext()) {
                if (t.isCommaAnd()) 
                    continue;
                if ((((g = (GeoReferent)com.pullenti.n2j.Utils.cast(t.getReferent(), GeoReferent.class)))) == null) 
                    break;
            ...
            if (i < 2) 
                continue;
            uni = (GeoReferent)com.pullenti.n2j.Utils.cast(ad.registerReferent(uni), GeoReferent.class);
        ...
        for(com.pullenti.ner.Token t = kit.firstToken; t != null; t = t.getNext()) {
            ...
            if (li != null) {
                if ((((rt = com.pullenti.ner.geo.internal.CityAttachHelper.tryAttachCity(li, ad, false)))) != null) {
                    ...
                    if (ad.getReferents().size() > 2000) 
                        break;
                    rt.referent = (GeoReferent)com.pullenti.n2j.Utils.cast(ad.registerReferent(rt.referent), GeoReferent.class);
                    kit.embedToken(rt);
                    t = rt;
                    isCityBefore = true;
                    newCities = true;
                    ...
                    while(true) {
                        ...
                        if (rr == null) 
                            break;
                        GeoReferent _geo = (GeoReferent)com.pullenti.n2j.Utils.cast(rr.referent, GeoReferent.class);
                        if (!_geo.isCity() && !_geo.isState()) 
                            nonRegistered.add(_geo);
                        else 
                            rr.referent = ad.registerReferent(_geo);
                    ...
                    rt = tryAttachTerritoryAfterCity(t, ad);
                    if (rt != null) {
                        rt.referent = ad.registerReferent(rt.referent);
                        kit.embedToken(rt);
                        t = rt;
        ...
        if (newCities && ad.localOntology.getItems().size() > 0) {
            for(com.pullenti.ner.Token t = kit.firstToken; t != null; t = t.getNext()) {
                ...
                if (t.getMorph()._getClass().isAdjective()) 
                    continue;
                GeoReferent _geo = (GeoReferent)com.pullenti.n2j.Utils.cast(li.get(0).item.referent, GeoReferent.class);
                if (_geo != null) {
                    com.pullenti.ner.ReferentToken rt = com.pullenti.ner.ReferentToken._new709(_geo, li.get(0).getBeginToken(), li.get(0).getEndToken(), t.getMorph());
                    if (rt.getBeginToken() == rt.getEndToken()) 
                        _geo.addName((((com.pullenti.ner.TextToken)com.pullenti.n2j.Utils.cast(t, com.pullenti.ner.TextToken.class))).term);
        ...
        for(com.pullenti.ner.Token t = kit.firstToken; t != null; t = t.getNext()) {
            ...
            for(; tt != null; tt = tt.getNext()) {
                ...
                geo1 = (GeoReferent)com.pullenti.n2j.Utils.cast(tt.getReferent(), GeoReferent.class);
                break;
            }
            if (geo1 == null) 
                continue;
            ...
            for(; ttt != null; ttt = ttt.getNext()) {
                ...
                if (ttt.getMorph()._getClass().isPreposition()) 
                    continue;
                geo2 = (GeoReferent)com.pullenti.n2j.Utils.cast(ttt.getReferent(), GeoReferent.class);
                break;
            }
            if (ttt != null && ttt.getWhitespacesBeforeCount() > 15) 
                geo2 = null;
            if (geo2 != null) {
                if ((comma1 && comma2 && com.pullenti.ner.geo.internal.GeoOwnerHelper.canBeHigher(t, tt)) && com.pullenti.ner.geo.internal.GeoOwnerHelper.canBeHigher(tt, ttt)) {
                    geo2.setHigher(geo1);
                    geo1.setHigher(_geo);
                    com.pullenti.ner.ReferentToken rt = com.pullenti.ner.ReferentToken._new709(geo2, t, ttt, ttt.getMorph());
                    kit.embedToken(rt);
                    t = rt;
                    goBack = true;
                    continue;
                }
                else if (com.pullenti.ner.geo.internal.GeoOwnerHelper.canBeHigher(ttt, tt)) {
                    if (com.pullenti.ner.geo.internal.GeoOwnerHelper.canBeHigher(t, ttt)) {
                        geo2.setHigher(_geo);
                        geo1.setHigher(geo2);
                        com.pullenti.ner.ReferentToken rt = com.pullenti.ner.ReferentToken._new709(geo1, t, ttt, t.getMorph());
                        kit.embedToken(rt);
                        t = rt;
                        goBack = true;
                        continue;
                    }
                    if (com.pullenti.ner.geo.internal.GeoOwnerHelper.canBeHigher(ttt, t) && com.pullenti.ner.geo.internal.GeoOwnerHelper.canBeHigher(t, tt)) {
                        _geo.setHigher(geo2);
                        geo1.setHigher(_geo);
                        com.pullenti.ner.ReferentToken rt = com.pullenti.ner.ReferentToken._new709(geo1, t, ttt, tt.getMorph());
                        kit.embedToken(rt);
                        t = rt;
                        goBack = true;
                        continue;
                    }
                    if (com.pullenti.ner.geo.internal.GeoOwnerHelper.canBeHigher(tt, t)) {
                        _geo.setHigher(geo1);
                        geo1.setHigher(geo2);
                        com.pullenti.ner.ReferentToken rt = com.pullenti.ner.ReferentToken._new709(_geo, t, ttt, t.getMorph());
                        kit.embedToken(rt);
                        t = rt;
                        goBack = true;
                        continue;
            ...
            if (com.pullenti.ner.geo.internal.GeoOwnerHelper.canBeHigher(t, tt) && ((!com.pullenti.ner.geo.internal.GeoOwnerHelper.canBeHigher(tt, t) || adj))) {
                geo1.setHigher(_geo);
                com.pullenti.ner.ReferentToken rt = com.pullenti.ner.ReferentToken._new709(geo1, t, tt, tt.getMorph());
            ...
            if (com.pullenti.ner.geo.internal.GeoOwnerHelper.canBeHigher(tt, t) && ((!com.pullenti.ner.geo.internal.GeoOwnerHelper.canBeHigher(t, tt) || inp))) {
                ...
                if (bra && tt.getNext() != null && tt.getNext().isChar(')')) 
                    tt = tt.getNext();
                com.pullenti.ner.ReferentToken rt = com.pullenti.ner.ReferentToken._new709(_geo, t, tt, t.getMorph());
                kit.embedToken(rt);
                t = rt;
                goBack = true;
                continue;
        ...
        for(int k = 0; k < nonRegistered.size(); k++) {
            ...
            for(int i = 0; i < (nonRegistered.size() - 1); i++) {
                if (geoComp(nonRegistered.get(i), nonRegistered.get(i + 1)) > 0) {
                    ...
                    com.pullenti.n2j.Utils.putArrayValue(nonRegistered, i, nonRegistered.get(i + 1));
                    com.pullenti.n2j.Utils.putArrayValue(nonRegistered, i + 1, v);
                }
            }
            if (!ch) 
                break;
        ...
        for(GeoReferent ng : nonRegistered) {
            for(com.pullenti.ner.Slot s : ng.getSlots()) {
                if (s.getValue() instanceof GeoReferent) {
                    if ((((GeoReferent)com.pullenti.n2j.Utils.cast(s.getValue(), GeoReferent.class))).getTag() instanceof GeoReferent) 
                        ng.uploadSlot(s, (GeoReferent)com.pullenti.n2j.Utils.cast((((GeoReferent)com.pullenti.n2j.Utils.cast(s.getValue(), GeoReferent.class))).getTag(), GeoReferent.class));
                }
            }
            GeoReferent rg = (GeoReferent)com.pullenti.n2j.Utils.cast(ad.registerReferent(ng), GeoReferent.class);
            if (rg == ng) 
                continue;
            ng.setTag(rg);
        ...
        for(com.pullenti.ner.Token t = kit.firstToken; t != null; t = t.getNext()) {
            ...
            if (_geo == null) 
                continue;
            _replaceTerrs((com.pullenti.ner.ReferentToken)com.pullenti.n2j.Utils.cast(t, com.pullenti.ner.ReferentToken.class));
        }
    }
    ...
    private static void _replaceTerrs(com.pullenti.ner.ReferentToken mt) {
        if (mt == null) 
            return;
        GeoReferent _geo = (GeoReferent)com.pullenti.n2j.Utils.cast(mt.referent, GeoReferent.class);
        if (_geo != null && (_geo.getTag() instanceof GeoReferent)) 
            mt.referent = (GeoReferent)com.pullenti.n2j.Utils.cast(_geo.getTag(), GeoReferent.class);
        if (_geo != null) {
            for(com.pullenti.ner.Slot s : _geo.getSlots()) {
                if (s.getValue() instanceof GeoReferent) {
                    GeoReferent g = (GeoReferent)com.pullenti.n2j.Utils.cast(s.getValue(), GeoReferent.class);
    ...
    private static int geoComp(GeoReferent x, GeoReferent y) {
        ...
        if (xcou > ycou) 
            return 1;
        return com.pullenti.n2j.Utils.stringsCompare(x.toString(true, com.pullenti.morph.MorphLang.UNKNOWN, 0), y.toString(true, com.pullenti.morph.MorphLang.UNKNOWN, 0), false);
    }
    ...
    private com.pullenti.ner.ReferentToken tryAttachTerritoryAfterCity(com.pullenti.ner.Token t, com.pullenti.ner.core.AnalyzerDataWithOntology ad) {
        if (t == null) 
            return null;
        GeoReferent city = (GeoReferent)com.pullenti.n2j.Utils.cast(t.getReferent(), GeoReferent.class);
        if (city == null) 
            return null;
        if (!city.isCity()) 
            return null;
    ...
    public com.pullenti.ner.ReferentToken processReferent(com.pullenti.ner.Token begin, com.pullenti.ner.Token end) {
        ...
        if (toks != null) {
            for(com.pullenti.ner.core.TerminToken tok : toks) {
                com.pullenti.ner.core.IntOntologyItem cit = (com.pullenti.ner.core.IntOntologyItem)com.pullenti.n2j.Utils.cast(tok.termin.tag, com.pullenti.ner.core.IntOntologyItem.class);
                if (cit == null) 
                    continue;
                GeoReferent city = new GeoReferent();
                city.addName(cit.getCanonicText());
                city.addTypCity(begin.kit.baseLanguage);
                return com.pullenti.ner.ReferentToken._new1201(city, tok.getBeginToken(), tok.getEndToken(), tok.getMorph(), begin.kit.getAnalyzerData(this));
            }
            return null;
        }
        com.pullenti.ner.core.AnalyzerDataWithOntology ad = (com.pullenti.ner.core.AnalyzerDataWithOntology)com.pullenti.n2j.Utils.cast(begin.kit.getAnalyzerData(this), com.pullenti.ner.core.AnalyzerDataWithOntology.class);
        if (!begin.getMorph()._getClass().isAdjective()) {
            com.pullenti.ner.TextToken te = (com.pullenti.ner.TextToken)com.pullenti.n2j.Utils.cast(begin, com.pullenti.ner.TextToken.class);
            if ((te.chars.isAllUpper() && te.chars.isCyrillicLetter() && te.getLengthChar() == 2) && te.getMorphClassInDictionary().isUndefined()) {
                ...
                for(com.pullenti.ner.core.IntOntologyItem t : ad.localOntology.getItems()) {
                    GeoReferent _geo = (GeoReferent)com.pullenti.n2j.Utils.cast(t.referent, GeoReferent.class);
                    if (_geo == null) 
                        continue;
                ...
                if (cou == 1) 
                    return com.pullenti.ner.ReferentToken._new115(geo0, begin, begin, ad);
            }
            ...
            if (tt0 != null && tt0.terminItem != null && com.pullenti.n2j.Utils.stringsEq(tt0.terminItem.getCanonicText(), "РАЙОН")) {
                ...
                if ((tt1 != null && tt1.chars.isCapitalUpper() && tt1.terminItem == null) && tt1.ontoItem == null) {
                    java.util.ArrayList<com.pullenti.ner.geo.internal.TerrItemToken> li = new java.util.ArrayList< >();
                    li.add(tt0);
                    li.add(tt1);
            ...
            if ((ctoks != null && ctoks.size() == 1 && ctoks.get(0).typ == com.pullenti.ner.geo.internal.CityItemToken.ItemType.NOUN) && com.pullenti.n2j.Utils.stringsEq(ctoks.get(0).value, "ГОРОД")) {
                int cou = 0;
                for(com.pullenti.ner.Token t = begin.getPrevious(); t != null; t = t.getPrevious()) {
                    ...
                    for(com.pullenti.ner.Referent g : geos) {
                        GeoReferent gg = (GeoReferent)com.pullenti.n2j.Utils.cast(g, GeoReferent.class);
                        if (gg != null) {
                            if (gg.isCity()) 
                                return com.pullenti.ner.ReferentToken._new1201(gg, begin, ctoks.get(0).getEndToken(), ctoks.get(0).getMorph(), ad);
                            if (gg.getHigher() != null && gg.getHigher().isCity()) 
                                return com.pullenti.ner.ReferentToken._new1201(gg.getHigher(), begin, ctoks.get(0).getEndToken(), ctoks.get(0).getMorph(), ad);
                        }
                    }
                }
            }
        ...
        if (tt == null || tt.ontoItem == null) {
            ...
            if ((tok != null && tok.get(0).item != null && (tok.get(0).item.referent instanceof GeoReferent)) && (((GeoReferent)com.pullenti.n2j.Utils.cast(tok.get(0).item.referent, GeoReferent.class))).isState()) 
                tt = com.pullenti.ner.geo.internal.TerrItemToken._new1112(tok.get(0).getBeginToken(), tok.get(0).getEndToken(), tok.get(0).item);
        }
        if (tt == null) 
            return null;
        if (tt.ontoItem != null) {
            ...
            if (res.getBeginToken() == res.getEndToken()) {
                ...
                if (mc.isAdjective()) {
                    GeoReferent _geo = (GeoReferent)com.pullenti.n2j.Utils.cast(tt.ontoItem.referent, GeoReferent.class);
                    if (_geo.isCity() || _geo.isState()) {
                    }
    ...
    public com.pullenti.ner.ReferentToken processCitizen(com.pullenti.ner.Token begin) {
        ...
        if (tok == null) 
            return null;
        GeoReferent geo0 = (GeoReferent)com.pullenti.n2j.Utils.cast(tok.termin.tag, GeoReferent.class);
        if (geo0 == null) 
            return null;
    ...
    public com.pullenti.ner.ReferentToken processOntologyItem(com.pullenti.ner.Token begin) {
        ...
        if (li != null && li.size() > 1 && li.get(0).typ == com.pullenti.ner.geo.internal.CityItemToken.ItemType.NOUN) {
            ...
            if (rt == null) 
                return null;
            GeoReferent city = (GeoReferent)com.pullenti.n2j.Utils.cast(rt.referent, GeoReferent.class);
    ...
    private static String[] ends_GeoAnalyzerDataWithOntology = new String[] {"КИЙ", "КОЕ", "КАЯ"};
    public GeoAnalyzer() {
        super();
    }
