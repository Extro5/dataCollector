...
public class Morphology {
    ...
    private static com.pullenti.morph.internal.InnerMorphology m_Inner = new com.pullenti.morph.internal.InnerMorphology();
     ...
     * @return последовательность результирующих лексем
     */
    public static java.util.ArrayList<MorphToken> process(String text, MorphLang lang, com.pullenti.n2j.ProgressEventHandler progress) {
        if (com.pullenti.n2j.Utils.isNullOrEmpty(text)) 
            return null;
    ...
    private static java.util.ArrayList<MorphWordForm> m_EmptyWordForms = new java.util.ArrayList< >();

    private static MorphMiscInfo m_EmptyMisc = new MorphMiscInfo();
    ...
    public static String getWordform(String word, MorphBaseInfo morphInfo) {
        if (morphInfo == null || com.pullenti.n2j.Utils.isNullOrEmpty(word)) 
            return word;
        MorphClass cla = morphInfo._getClass();
        if (cla.isUndefined()) {
            MorphBaseInfo mi0 = getWordBaseInfo(word, new MorphLang(null), false);
            if (mi0 != null) 
                cla = mi0._getClass();
        }
        ...
        return (String)com.pullenti.n2j.Utils.notnull(m_Inner.getWordform(word, cla, morphInfo.getGender(), morphInfo.getCase(), morphInfo.getNumber(), morphInfo.getLanguage(), (MorphWordForm)com.pullenti.n2j.Utils.cast(morphInfo, MorphWordForm.class)), word);
    }
     ...
     */
    public static MorphBaseInfo getWordBaseInfo(String word, MorphLang lang, boolean isCaseNominative) {
    ...
    public static String convertAdverbToAdjective(String adverb, MorphBaseInfo bi) {
        ...
        MorphBaseInfo bi1 = getWordBaseInfo(var1, new MorphLang(null), false);
        MorphBaseInfo bi2 = getWordBaseInfo(var2, new MorphLang(null), false);
        String var = var1;
        if (!bi1._getClass().isAdjective() && bi2._getClass().isAdjective()) 
            var = var2;
        if (bi == null) 
            return var;
        return (String)com.pullenti.n2j.Utils.notnull(m_Inner.getWordform(var, MorphClass.ADJECTIVE, bi.getGender(), bi.getCase(), bi.getNumber(), MorphLang.UNKNOWN, null), var);
    }
    public Morphology() {
    }
}
